"""问题一求解：有效视觉响应提取、曲线拟合与结果汇总。

产出：
  Q1/figures/fig1_erp_curve.{svg,pdf,png}   三导联去噪前后事件相关电位曲线与高斯分量拟合
  Q1/figures/fig2_artifact_interval.{svg,pdf,png}  单试次峰值幅值的区间点图（去噪前后对照）
  Q1/figures/fig3_artifact_count.{svg,pdf,png}     各记录伪迹处理计数
...
from compare_denoise import artifact_suppression  # noqa: E402

FS = E.FS
N_BASE = int(round(E.EPOCH_PRE * FS))
P300 = slice(N_BASE + int(round(0.25 * FS)), N_BASE + int(round(0.50 * FS)))
...
def fit_components(t, erp):
    ...
    for k in range(3):
        ...
        eamp = perr[1 + 3 * k]
        comps.append({"amplitude_uV": round(float(amp), 4), "amplitude_se_uV": round(float(eamp), 4),
                      "latency_ms": round(float(mu) * 1000, 2), "width_ms": round(float(sigma) * 1000, 2)})
    comps.sort(key=lambda c: c["latency_ms"])
    return {"components": comps, "r2": round(1.0 - ss_res / ss_tot, 5) if ss_tot > 0 else None,
            "rmse_uV": round(float(np.sqrt(ss_res / erp.size)), 4),
            "baseline_uV": round(float(popt[0]), 4), "fitted": fitted}
...
def figure_erp(t, curves, out_dir):
    ...
    for ch in ("Fz", "F3", "F4"):
        ax.plot(t * 1000, curves[ch]["fitted"], color=colors[ch], lw=1.0, ls=(0, (5, 3)),
                alpha=0.85, zorder=2)
    ax.plot([], [], color=P.PALETTE["neutral_dark"], lw=1.0, ls=(0, (5, 3)),
            label="三高斯分量拟合")
    ...
    ax.text(375, bot - 0.02 * (top - bot), "P300 窗", ha="center", va="bottom",
            fontsize=7, color=P.PALETTE["neutral_dark"])
...
def figure_interval(per_dataset, out_dir):
    """图2：单试次峰值幅值的区间点图（去噪前后对照）。"""
    font = P.apply_style(8.0)
    fig = plt.figure(figsize=(P.mm_to_inch(89.0), P.mm_to_inch(62.0)))
    ax = fig.add_axes([0.22, 0.17, 0.75, 0.70])
    keys = ["A1", "A2", "B1", "B2"]
    off = 0.15
    for i, key in enumerate(keys):
        rec = per_dataset[key]
        for j, (tag, color, label) in enumerate((
                ("raw", P.PALETTE["neutral_mid"], "去噪前"),
                ("den", P.PALETTE["blue_main"], "去噪后"))):
            vals = rec[tag]
            ...
            ax.scatter([med], [y], s=24, color=color, zorder=3,
...
def figure_count(counts, out_dir):
    """图3：各记录的伪迹处理计数（零基线绝对计数比较，符合少用柱形的例外条件）。"""
    font = P.apply_style(8.0)
    fig = plt.figure(figsize=(P.mm_to_inch(89.0), P.mm_to_inch(62.0)))
    ...
    for bars in (b1, b2):
        for rect in bars:
            h = rect.get_height()
            ax.annotate(f"{h:.0f}", (rect.get_x() + rect.get_width() / 2, h), xytext=(0, 2),
...
def main() -> int:
    ...
    for spec in E.DATASETS:
        ...
        for ch in E.EEG_CHANNELS:
            ...
            for label, tag in ((-1, "left"), (1, "right")):
                ...
                cond[tag] = {
                    "trials": int(sel.sum()),
                    "p300": bootstrap_peak(den[sel], P300),
                    "early": bootstrap_peak(den[sel], EARLY),
                    "late_mean_uV": round(float(den[sel][:, LATE].mean()), 4),
                    "reliability": round(float(np.corrcoef(den[sel][0::2].mean(0)[N_BASE:],
                                                           den[sel][1::2].mean(0)[N_BASE:])[0, 1]), 4),
                }
            entry["channels"][ch] = {
                ...
                "significant_points_post800": int(np.count_nonzero(mask[post])),
                "p300_window_ms": [250, 500],
                "gaussian_fit": {k: v for k, v in fit.items() if k != "fitted"},
                "left": cond["left"], "right": cond["right"],
                "laterality_index": round(
                    (cond["right"]["p300"]["amplitude_uV"] - cond["left"]["p300"]["amplitude_uV"])
                    / (abs(cond["right"]["p300"]["amplitude_uV"]) + abs(cond["left"]["p300"]["amplitude_uV"]) + 1e-9), 4),
            }
        ...
        print(f"[{key}] 拟合 R²=" + " ".join(
            f"{ch}:{entry['channels'][ch]['gaussian_fit']['r2']}" for ch in E.EEG_CHANNELS))

    # 伪迹处理计数：从派生数据与审计记录读取真实值
    ...
    results["figures"] = {
        "fig1_erp_curve": {"paths": [str(p.relative_to(PROJECT)).replace("\\", "/") for p in p1],
                           "dataset_shown": "A2", "note": "曲线取自 A 组项目二记录，该记录响应最强"},
        "fig2_artifact_interval": {"paths": [str(p.relative_to(PROJECT)).replace("\\", "/") for p in p2]},
        "fig3_artifact_count": {"paths": [str(p.relative_to(PROJECT)).replace("\\", "/") for p in p3]},
    }
