...
- **一键安装的官方本体升级到 `0.1.5-rc.2`，修复版本号滞后**：此前安装脚本虽已在 `0.1.5-rc.2` 上开发验证，锁定的却还是旧版 `0.1.2-rc.1`——全新电脑装出来会与内置的模型通道插件不匹配（按上游文档会直接启动失败），已升级到旧版的老用户重跑一次完整安装即可升回新版。日常已正常使用的用户无需任何操作。配套的模型目录适配脚本也已在新版官方本体上实测通过（OpenCode Go 自动加入 DeepSeek V4.1 Flash，重复运行自动跳过）。

## 最近更新（2026-09-20，一键安装修复）

- **修复全新安装必然失败的问题**：此前 `metamath-harness-20260920b.zip` 的安装脚本存在三处缺陷，新电脑按 README 双击 `install.cmd` 安装会直接报错：①安装脚本引用的模型目录适配脚本在上一次打包时被误删，现已恢复——“DeepSeek 官方与 OpenCode Go 自动加入 DeepSeek V4.1 Flash”的功能照常生效；②脚本里残留的桌宠插件构建步骤未随桌宠下架清理，现已删除；③Cline 免费通道和 Qoder 两个新插件在压缩包里缺少构建产物，装完后模型选择器会报“模块不存在”，安装脚本现在会自动构建这两个插件。请重新下载最新 ZIP 覆盖安装；此前已正常运行的旧安装不受影响。
- **桌宠移除原因补充说明**：移除桌宠是为避免常驻动画渲染造成卡顿、优化使用体验的有意措施，并非文件遗漏；该说明已写入本仓库 AGENTS.md，避免 AI 助手下载后误报缺失。

## 最近更新（2026-09-20，桌宠下架）

- **桌宠插件已从产品中移除**：应维护者决定，本套件不再内置桌面宠物（含表情包配图功能）；聊天界面右上角不再出现宠物入口，设置页的桌宠配置卡片也已下架。此前版本安装过的用户在覆盖更新后宠物会自动消失，无需手工清理；如仍需使用桌宠，请继续使用 2026-09-20 之前的发行包，或单独安装上游插件（素材来源 [PC2005-cloud/dsh-pet](https://github.com/PC2005-cloud/dsh-pet)，素材禁止商用）。本次移除不影响其他模型通道与论文功能。

## 最近更新（2026-09-20）

- **新增 Cline 免费模型通道（多 Key 轮询 + 故障自动切换）**：在「设置 → 插件 → Cline Free API Key」卡片把一个或多个 Cline API Key 逐行粘贴保存（最多 32 把，自动去重），保存后最迟约 1 分钟进入轮询；每个请求自动换用不同 Key，遇到 401/403/429 会把失败 Key 冷却一段时间并自动换下一把重试；已开始输出内容后的失败不会重放，避免重复回复。卡片会如实显示保存/清除的每一步结果，写入失败不会误报成功。Key 只保存在本机 DSH 凭据服务，不进入会话、日志或仓库。免费模型目录来自 Cline 官方免费接口，无需付费。模型通道插件来源：[jiesou/dsh-cline-free-provider](https://github.com/jiesou/dsh-cline-free-provider) v0.1.19 本地派生（MIT 许可证）；管理卡片为本仓库原创。
- **新增 Qoder 模型通道（PAT 登录）**：在设置页 Qoder 卡片填入 Qoder 平台的 PAT 即可在模型选择器使用 Qoder 分组模型；凭据只保存在本机 DSH 凭据服务。来源：[mo-n/dsh-provider-qoder](https://github.com/mo-n/dsh-provider-qoder) v0.3.0（MIT 许可证）。
- **桌宠新增表情包配图**：桌宠聊天的碎碎念和对话配图功能上线，素材为本仓库维护者自制的表情包（内置 27 张）；在设置页桌宠卡片打开「碎碎念配图 / 对话配图」开关即可，不开则完全不变。
- **数学建模论文门禁全面收紧，AI 更难“糊弄过关”**：图型重复策略现在提供“默认少重复 / 模型自行分析 / 禁止重复”三档，同数据只换配色的凑图会被点名；同名导出文件重复登记会被判为清单错误而不是让 AI 重画正确的图；文件名带“路线图”的真实数据图不再被误排除。华为杯论文新增“符号说明”位置硬验收：不仅查章节源码结构，还会编译 PDF 后按实际页面坐标核对“符号说明标题 → 符号表 → 问题一”的阅读顺序，标题孤悬、表格漂移、数据分析插队都会被拦下并给出具体修复提示；编译产物过期时拒绝复用旧结果。
- **首页“大道至简”品牌恢复**：新版官方界面下首页主视觉重新显示“大道至简”金属艺术字（已实测验证），预览版徽章与官方占位文字不再出现；官方升级后再出现时随本套件更新自动恢复。
- **清理了仓库中不属于产品的历史文件**：此前误上传的一个赛题工作区目录已从仓库最新版本移除；请勿在任何公开仓库存放个人赛题、论文与数据。
...
- **会话深链：一键复制会话链接、直达任意历史会话**：每个会话的标题栏新增「复制会话链接」按钮，点击后把 `dsh://session/<会话ID>` 深链复制到剪贴板；在浏览器地址栏打开 `http://127.0.0.1:3080/s/<会话ID>`（或把链接粘贴到任意对话）即可直达并引用该会话。链接只在安装了本套件的电脑上有效，不含任何账号或密钥信息。来源：[PwnKY/dsh-session-link](https://github.com/PwnKY/dsh-session-link) v0.1.4，MIT 许可证。

<p align="center">
  <img src="github展示/会话-复制会话链接.png" width="70%" alt="会话头部的复制会话链接按钮" />
</p>

- **识图开关回归 + 升级守护脚本**：在「设置 → 模型 → 自定义提供方」的模型行「容量」折叠区，勾选「识图（图片输入）」即可让该模型接收图片输入（取消勾选恢复默认）。官方升级会覆盖该开关，随仓库新增 `scripts/patch-settings-models-vision.py`：升级后运行 `python scripts/patch-settings-models-vision.py` 一键恢复，脚本可重复运行（已恢复时提示在位、不重复改动）。

<p align="center">
  <img src="github展示/设置-模型-识图开关.png" width="70%" alt="自定义提供方模型行的识图开关" />
</p>

- **OpenCode 订阅会话修复（原创插件）**：OpenCode 订阅用户此前会偶发「MissingSessionID」导致请求被拒。新增本地原创插件 `dsh-opencode-session`（自动随安装脚本构建，无任何配置），为每个会话稳定补全会话请求头，不同会话相互隔离；不支持的场景会在本地直接报错而不是发出注定失败的请求。该插件绑定官方 `0.1.2-rc.1` 适配器版本，升级官方本体后若报指纹不匹配，等待本仓库更新即可。
- **识图工具更稳健**：本地图片分析不再只看扩展名——没有扩展名的 PNG/JPEG/GIF/WebP 也能正确识别；扩展名与实际内容不一致时会明确报错，模型不会被错误格式的图片误导；全部模型失败时的错误信息会列出每个模型的具体原因，且不回显任何密钥。
