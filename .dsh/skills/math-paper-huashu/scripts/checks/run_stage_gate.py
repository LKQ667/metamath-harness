...
def launcher_source(default_args: list[str]) -> str:
    ...
    return (
        "#!/usr/bin/env python\n"
        "# -*- coding: utf-8 -*-\n"
        '"""由 math-paper-huashu 自动生成；不要改写或复制门禁逻辑。"""\n'
        "import hashlib\n"
        "from pathlib import Path\n"
        "import subprocess\n"
        "import sys\n\n"
        ...
        "candidates = (\n"
        "    Path.home() / '.codex' / 'skills' / 'math-paper-huashu' / 'scripts' / 'checks' / 'run_stage_gate.py',\n"
        "    Path.home() / '.trae-cn' / 'skills' / 'math-paper-huashu' / 'scripts' / 'checks' / 'run_stage_gate.py',\n"
        ")\n"
        ...
        "if RUNNER is None:\n"
        "    raise SystemExit('门禁源缺失或版本已变化，请从当前 math-paper-huashu Skill 重新运行 --init。')\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="运行 math-paper-huashu 逐阶段硬门禁")
