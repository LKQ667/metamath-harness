...
def paper_body_region(text: str) -> tuple[str, list[str]]:
    ...
    title = text.find("\\biaoti{") if huashu else text.find("{\\titlefont", start)
    abstract = text.find("\\begin{abstract}", start) if huashu else text.find("{\\sectiontitlefont 摘要}", start)
    bibliography = text.find("\\begin{thebibliography}", start)
    bibliography_end = text.find("\\end{thebibliography}", bibliography)
    appendices = text.find("\\begin{appendixx}", appendix) if huashu else text.find("\\begin{appendices}", appendix)
    ...
    if title == -1 or (not huashu and not (start < title < end)):
    ...
    if appendices == -1 or not (appendix < appendices):
        errors.append("`appendix:start` 必须位于模板附录环境之前。")
