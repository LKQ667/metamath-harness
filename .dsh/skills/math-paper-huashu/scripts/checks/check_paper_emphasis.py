...
def main() -> int:
    parser = project_arg("检查论文摘要与正文的选择性粗体强调")
    ...
    abstract_title = text.find("\\begin{abstract}") if huashu else text.find(ABSTRACT_TITLE)
    abstract_end = text.find("\\end{abstract}", abstract_title + 1) if huashu else text.find(ABSTRACT_END, abstract_title + 1)
    ...
    abstract_start = abstract_title + len("\\begin{abstract}") if huashu else abstract_title + len(ABSTRACT_TITLE)
    ...
    for index, problem in enumerate(problems):
        ...
        section_end = (
            problems[index + 1].start()
            if index + 1 < len(problems)
            else (len(abstract) if huashu else abstract.rfind("关键词"))
        )
    ...
    if huashu:
        keyword_match = re.search(r"\\keyword\s*\{([^{}]+)\}", text)
        if keyword_match is None or not keyword_match.group(1).strip():
            errors.append("华数杯主稿缺少非空关键词配置。")
    elif keyword_pos == -1:
    ...
    body_start = abstract_end + (len("\\end{abstract}") if huashu else len(ABSTRACT_END))
