...
Use the run image backend's edit operation for background repair or clean bases when (classic backends: `editppt image edit --image <source.png>`; `dsh-current`: one `editable_ppt_image {action:"edit"}` call with `source.png` in `referenceImages`):
...
Every non-text foreground visual object must be separated through the asset-sheet workflow of the run image backend (classic backends: `editppt image edit --image <source.png>`; `dsh-current`: `editable_ppt_image {action:"edit"}` with the source image as reference), including:
