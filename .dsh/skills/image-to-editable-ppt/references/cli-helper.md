...
`editppt image` automatically chooses the image backend: Codex OAuth first, then OpenAI-compatible API credentials from `~/.editppt/config.yaml` or environment variables if OAuth is unavailable. Exception: in a `dsh-current` run (the DSH-only editable-PPT contract) every `editppt image generate/edit` call is hard-blocked before any credential is read (`dsh_current_cli_forbidden`); those runs call the DSH host tool `editable_ppt_image` instead, and only `editppt image import` / `process-sheet` remain as the registration and processing surface (backend dispatch is authoritative in `SKILL.md` Entry Contract).
...
DSH-only (dsh-current) form — after the DSH tool `editable_ppt_image {action:"status"}` returned the connection to pin:
...
This writes the `dsh-current` backend contract (run-start-pinned connection, `allow_codex:false`, `fallback_policy:"none"`) into `deck_manifest.json` and every `page_request.json` in one step; no follow-up `editppt config` or backend command is needed.
...
These commands call the selected image backend: Codex OAuth first, then a configured OpenAI-compatible API fallback. In `dsh-current` runs they are blocked (`dsh_current_cli_forbidden`) before any credential is read — perform the equivalent generation/edit with the DSH tool `editable_ppt_image` instead (`{"action":"generate"|"edit","connectionId":"<run-pinned id>","prompt":"...","referenceImages":[...],"maskImage":"...","size":"auto","quality":"high","outputPath":"pages/page_NNN/assets/<name>.png","authorizePaid":true}`), one image per serial call, deterministic path, no rename and no fallback on failure. In a network-restricted runtime, request approval before the call and state that only task-local prompts plus required page images/masks/references are uploaded for the current conversion.
...
In `dsh-current` runs, `--metadata-file` is required and carries the DSH tool's sidecar metadata for provenance validation (connection must equal the run-pinned id; hashes/protocol/model must match; secrets are rejected):

```bash
editppt image import pages/page_001 \
  --job-id icon-sheet \
  --source-image <editable_ppt_image result.file> \
  --dest assets/icon-sheet.png \
  --role asset_sheet \
  --prompt-file prompts/icon-sheet.md \
  --metadata-file <editable_ppt_image result.metadataFile> \
