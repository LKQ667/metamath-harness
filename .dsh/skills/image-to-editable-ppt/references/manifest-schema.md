...
Two contract shapes exist. The classic backends (`editppt-image-cli`, `openai-compatible-api`) keep the original fields (`tool_call`, `fallback_command`, `runtime_home`, `requires_openai_api_key`, Codex-first `handoff_rule`). The DSH-only `dsh-current` contract is fixed:
...
It never contains a Base URL, credential reference, key, or token; `connection_id` is pinned at run start and every page worker must reuse it verbatim.
...
In `dsh-current` runs, each recorded job additionally carries `metadata_file` (page-relative sidecar path), `metadata_sha256`, `connection_id`, `model`, and `protocol`, written only after `editppt image import` validates the DSH sidecar metadata against the run contract. The jobs file never stores Base URLs, credential references, keys, tokens, or prompt bodies.
