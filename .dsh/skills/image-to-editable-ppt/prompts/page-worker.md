...
1. Every non-text foreground visual object must be separated through the asset-sheet workflow of the run's image backend (defined below; for classic backends that is `editppt image edit --image <source.png>`) per page-decision-tree.md section 2. There is no fallback: no native-shape/emoji/text-symbol approximation, no direct source.png crops, no downgrade to a warning.
...
Image backend (dispatch authority: `SKILL.md` Entry Contract; rendered per run by `scripts/build-page-worker-prompt.py`):
