...
from configure_image_backend import BACKEND_CHOICES, dsh_backend_contract
...
def page_request(run_dir, deck, page):
    ...
    if isinstance(deck.get("image_backend"), dict):
...
def upgrade_deck_manifest(deck_path, max_concurrent_pages, image_backend=None):
    ...
    if image_backend is not None:
        deck["image_backend"] = image_backend
...
def main():
    ...
    image_backend = None
    if args.image_backend == "dsh-current":
        # 在创建运行目录前验证完整契约；失败时不得留下可回退到 Codex 的半成品运行。
        image_backend = dsh_backend_contract(SimpleNamespace(
    ...
    deck_path = normalize_inputs(
        ...
        dpi=args.dpi,
        initial_deck_fields={"image_backend": image_backend} if image_backend is not None else None,
    )
    run_dir = deck_path.parent
    if not (run_dir / "pages").exists():
        raise SystemExit(f"Input normalization did not create pages/: {run_dir}")
    deck = upgrade_deck_manifest(deck_path, args.max_concurrent_pages, image_backend=image_backend)
    print(deck_path)
