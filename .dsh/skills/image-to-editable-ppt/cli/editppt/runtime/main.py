...
def cmd_prepare(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "image_backend", None) == "dsh-current":
        argv.extend(["--image-backend", "dsh-current"])
        for flag, value in (
            ("--connection-id", getattr(args, "connection_id", None)),
            ("--connection-name", getattr(args, "connection_name", None)),
            ("--image-model", getattr(args, "image_model", None)),
            ("--image-protocol", getattr(args, "image_protocol", None)),
        ):
    ...
    return cmd_backend(
        argparse.Namespace(
            ...
            mode=getattr(args, "image_backend", None) or "editppt-image-cli",
            tool_name=None,
            tool_call=None,
            ...
            image_protocol=getattr(args, "image_protocol", None),
        )
    )
...
def cmd_backend(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "handoff_rule", None):
        argv.extend(["--handoff-rule", args.handoff_rule])
    if getattr(args, "connection_id", None):
        argv.extend(["--connection-id", args.connection_id])
    if getattr(args, "connection_name", None):
        argv.extend(["--connection-name", args.connection_name])
    if getattr(args, "image_protocol", None):
        argv.extend(["--image-protocol", args.image_protocol])
...
""",
    ...
    prepare.add_argument(
        "--image-backend",
        choices=["editppt-image-cli", "openai-compatible-api", "dsh-current"],
        default=None,
        help="Run-level image backend contract. Default: editppt-image-cli. dsh-current pins the DSH connection returned by editable_ppt_image status.",
    )
    prepare.add_argument("--connection-id", metavar="ID", help="dsh-current: run-pinned DSH image connection id (from editable_ppt_image status).")
    prepare.add_argument("--connection-name", metavar="NAME", help="dsh-current: non-secret connection display name.")
    prepare.add_argument("--image-model", metavar="MODEL", help="dsh-current: model of the pinned connection (e.g. gpt-image-2).")
    prepare.add_argument("--image-protocol", metavar="PROTOCOL", help="dsh-current: verified protocol of the pinned connection (e.g. openai-images).")
...
""",
    ...
    backend.add_argument("--mode", choices=["editppt-image-cli", "openai-compatible-api", "dsh-current"], default="editppt-image-cli", help="Image backend mode. Defaults to the unified editppt image CLI contract. dsh-current routes all image calls to the DSH editable_ppt_image tool with the run-pinned connection.")
    ...
    backend.add_argument("--model", metavar="MODEL", help="Image model label for API/CLI fallback, or the pinned DSH connection model for dsh-current.")
    ...
    backend.add_argument("--connection-id", metavar="ID", help="dsh-current: run-pinned DSH image connection id.")
    backend.add_argument("--connection-name", metavar="NAME", help="dsh-current: non-secret connection display name.")
    backend.add_argument("--image-protocol", metavar="PROTOCOL", help="dsh-current: verified protocol of the pinned connection.")
    backend.add_argument("--handoff-rule", metavar="TEXT", help="Override the handoff rule note for non-dsh backends.")
