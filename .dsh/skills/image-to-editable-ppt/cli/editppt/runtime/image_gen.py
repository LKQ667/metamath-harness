...
DSH_CURRENT_CLI_FORBIDDEN = "dsh_current_cli_forbidden"


def _deck_manifest_above_output(out_value: str) -> Optional[Path]:
...
def _guard_not_dsh_current_run(out_value: str) -> None:
    """dsh-current 运行内禁止本 CLI 触图：失败必须发生在任何 Codex/认证/API/网络路径之前。"""
    manifest = _deck_manifest_above_output(out_value)
    if manifest is None:
        return
    try:
        deck = json.loads(manifest.read_text(encoding="utf-8"))
    except (OSError, ValueError) as exc:
        _die(f"输出路径上方发现无法解析的 deck_manifest.json：{manifest}（{exc}）")
    backend = deck.get("image_backend") if isinstance(deck, dict) else None
    if isinstance(backend, dict) and backend.get("backend_id") == "dsh-current":
        _die(
            f"{DSH_CURRENT_CLI_FORBIDDEN}：本运行已锁定 DSH 当前生图连接（dsh-current，"
            f"connection_id={backend.get('connection_id', '未知')}）。所有生图/编辑必须调用 DSH 的 "
            "editable_ppt_image 工具并携带运行开始时锁定的 connectionId；禁止使用 editppt image "
            "generate/edit 代替，禁止 Codex/OpenAI API 后端消耗，禁止更换后端或要求重新输入 Key。"
        )
...
""",
    ...
    if not args.dry_run:
        _guard_not_dsh_current_run(args.out)
