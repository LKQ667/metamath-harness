...
def normalize_inputs(inputs, out_root="output/image-to-editable-ppt", job_dir=None, dpi=180, initial_deck_fields=None):
    ...
    if initial_deck_fields:
        # dsh-current 必须在任何耗时复制/渲染前留下 fail-closed 标记，硬中断也不能回退 Codex。
        job_dir.mkdir(parents=True, exist_ok=True)
        (job_dir / "deck_manifest.json").write_text(
            json.dumps({"schema_version": 1, "run_status": "preparing", **initial_deck_fields}, ensure_ascii=False, indent=2) + "\n",
            encoding="utf-8",
        )
