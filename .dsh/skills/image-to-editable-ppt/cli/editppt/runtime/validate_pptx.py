...
def pixel_authoring_violations(manifest):
    ...
    for index, item in enumerate(manifest.get("shapes", [])):
        if item.get("type") == "line":
            if "points_px" not in item and "bezier_px" not in item:
                violations.append(
                    {
                        "field": f"shapes[{index}].points_px",
                        "reason": "line shapes must use source-image pixel endpoints or bezier_px control points",
                    }
                )
            elif "bezier_px" in item:
                points = item.get("bezier_px")
                if not isinstance(points, list) or len(points) != 4:
                    violations.append(
                        {
                            "field": f"shapes[{index}].bezier_px",
                            "reason": "bezier line shapes must use four source-image pixel control points",
                        }
                    )
