...
def normalize_position_item(manifest, item):
    ...
    if "bezier_px" in item:
        points = [(float(point[0]), float(point[1])) for point in item["bezier_px"]]
        if len(points) == 4:
...
def arrow_xml(value, tag):
    arrow = str(value or "").strip().lower()
    if arrow not in {"triangle", "arrow", "end"}:
        return ""
    return f'<a:{tag} type="triangle"/>'
...
def shape_line_xml(stroke, width, dash=None, start_arrow=None, end_arrow=None):
    ...
    start_arrow_xml = arrow_xml(start_arrow, "headEnd")
    end_arrow_xml = arrow_xml(end_arrow, "tailEnd")
    return (
        f'<a:ln w="{int(float(width or 1) * 12700)}">'
        f'<a:solidFill><a:srgbClr val="{hex_color(stroke)}"/></a:solidFill>'
        f"{dash_xml}"
        f"{start_arrow_xml}{end_arrow_xml}"
        "</a:ln>"
    )
...
def text_box_xml(idx, item):
    ...
    font = xml_text(item.get("font", "Microsoft YaHei"))
    align = {"left": "l", "center": "ctr", "right": "r"}.get(
        item.get("align", "left"), item.get("align", "l")
    )
    anchor = {"top": "t", "mid": "ctr", "center": "ctr", "bottom": "b"}.get(
        item.get("valign", "top"), item.get("valign", "t")
    )
    ...
    def run_xml(run):
        ...
        run_font = xml_text(run.get("font", item.get("font", "Microsoft YaHei")))
...
def shape_xml(idx, item):
    ...
    line_info = item.get("line") if isinstance(item.get("line"), dict) else {}
    line = shape_line_xml(
        item.get("stroke", "#000000"),
        stroke_width,
        item.get("dash"),
        item.get("start_arrow") or item.get("arrow_start") or line_info.get("start_arrow") or line_info.get("arrow_start"),
        item.get("end_arrow") or item.get("arrow_end") or line_info.get("end_arrow") or line_info.get("arrow_end"),
    )
...
def layer_z_index(item, default_value):
    if item.get("role") == "connector" or item.get("bezier_px") or item.get("points_px") or item.get("type") == "line":
        return max(float(item.get("z_index", 240)), 240.0)
    if item.get("z_index") is not None:
        return float(item.get("z_index", default_value))
    return float(default_value)


def custom_bezier_geometry_xml(item):
    points = [(float(point[0]), float(point[1])) for point in item.get("bezier_px", [])]
    if len(points) != 4:
    ...
    start_x, start_y = rel_coord(points[0])
    controls = []
    for point in points[1:]:
        x, y = rel_coord(point)
        controls.append(f'<a:pt x="{x}" y="{y}"/>')
    return (
        '<a:custGeom><a:avLst/><a:gdLst/><a:ahLst/><a:cxnLst/>'
        '<a:rect l="l" t="t" r="r" b="b"/>'
        '<a:pathLst><a:path w="21600" h="21600">'
        f'<a:moveTo><a:pt x="{start_x}" y="{start_y}"/></a:moveTo>'
        "<a:cubicBezTo>"
        + "".join(controls)
...
def slide_xml(manifest):
    ...
    for index, item in enumerate(manifest.get("shapes", [])):
        layered.append((layer_z_index(item, 100), index, "shape", item, None))
    for rel_index, item in enumerate(manifest.get("images", []), start=1):
        layered.append((layer_z_index(item, 200), rel_index, "image", item, f"rId{rel_index + 1}"))
    for index, item in enumerate(manifest.get("text_boxes", [])):
        layered.append((layer_z_index(item, 300), index, "text", item, None))
...
def render_preview(manifest, manifest_path, out_path):
    ...
    layered = []
    for index, item in enumerate(manifest.get("shapes", [])):
        layered.append((layer_z_index(item, 100), index, render_shape, item))
    for index, item in enumerate(manifest.get("images", [])):
        layered.append((layer_z_index(item, 200), index, render_image, item))
    for index, item in enumerate(manifest.get("text_boxes", [])):
        layered.append((layer_z_index(item, 300), index, render_text, item))
