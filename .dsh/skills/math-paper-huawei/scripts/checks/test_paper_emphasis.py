...
\begin{document}
{\sectiontitlefont 摘要}
\paperstrong{针对问题一}，建立\paperstrong{组合优化模型}，由约束关系确定可行域，并使用独立样本检验求解结果。计算表明模型在给定边界内保持稳定，最终得到\paperstrong{目标值为 12.5}。
{\keywordfont \paperstrong{关键词：}} \paperstrong{组合优化}；\paperstrong{灵敏度分析}\label{abstract:end}
\newpage
\section{问题重述}
正文使用变量、公式和结果解释推进论证。该段保留足够的普通文字，用于确认少量关键短语强调不会被误判。模型的核心判据为\paperstrong{误差不超过 2\%}，其余内容保持普通字重。
...
class PaperEmphasisTests(unittest.TestCase):
    def run_case(self, tex: str) -> subprocess.CompletedProcess[str]:
        with tempfile.TemporaryDirectory(prefix="math-paper-emphasis-") as tmp:
            paper = Path(tmp) / "论文"
            paper.mkdir()
    ...
    def test_problem_label_must_be_isolated(self) -> None:
        bad = GOOD_TEX.replace(r"\paperstrong{针对问题一}", r"\paperstrong{针对问题一，建立}")
    ...
    def test_full_sentence_with_external_period_is_rejected(self) -> None:
        bad = GOOD_TEX.replace(
            "正文使用变量、公式和结果解释推进论证。",
            r"\paperstrong{正文使用变量、公式和结果解释推进论证}。",
        )
