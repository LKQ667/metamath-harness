...
def main() -> int:
    ...
    abstract_title = text.find("\\begin{abstract}") if huawei else text.find(ABSTRACT_TITLE)
    abstract_end = text.find("\\end{abstract}", abstract_title + 1) if huawei else text.find(ABSTRACT_END, abstract_title + 1)
    ...
    abstract_start = abstract_title + len("\\begin{abstract}") if huawei else abstract_title + len(ABSTRACT_TITLE)
    ...
    for index, problem in enumerate(problems):
        ...
        section_end = (
            ...
            else (len(abstract) if huawei else abstract.rfind("关键词"))
        )
    ...
    body_start = abstract_end + (len("\\end{abstract}") if huawei else len(ABSTRACT_END))
