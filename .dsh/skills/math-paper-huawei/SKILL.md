...
这是一个面向华为杯（中国研究生数学建模竞赛）的全自动总技能，合并 step0 到 step5 的核心流程。项目根目录以卡片 `output_dir` 或用户指定位置为准，使用时以本技能为主线，按阶段推进到最终论文成稿。技能内置华为杯 GMCMthesis LaTeX 模板（`laTeX-模板-GMCMthesis/`）与 2023、2024 年往届优秀论文（`华为杯优秀论文_2023_2024/`，按 A–F 题分组）。
...
- 论文正文从标题页开始，包括标题、摘要、关键词、问题重述至参考文献的全部内容；只排除附录中的支撑材料文件目录和代码。正文默认以 16–20 页为质量目标，并遵守华为杯“正文不超过 20 页、附录不超过 10 页”的官方要求；若用户明确要求更高正文页数，以用户要求为准但不得突破官方上限；页数不足时不得通过扩充参考文献或新增章节凑页。
...
- 摘要与关键词必须全部位于第一页，`\keywords{}` 之后的 `\label{abstract:end}` 编译页码必须为 1；摘要有效文字不得少于 800 字，建议 850–1050 字，在不跨页的前提下尽量铺满第一页。必须使用 GMCMthesis 模板原生 `abstract` 环境与 `\keywords{}` 命令，禁止另造摘要环境、缩小字号、压缩行距或用负间距规避门禁。
...
- 当卡片 `reference_excellent_papers=true` 或用户要求参考往届作品时，只允许阅读技能内置 `华为杯优秀论文_2023_2024/` 中对应题号的优秀论文，用于校准完成度、章节深度、图表密度与写作风格；严禁复制其文字、公式、图表、数据或结论，引用其方法思路必须在参考文献中如实登记。
- 本技能内置可跨电脑分发的华为杯 LaTeX 模板：主稿 `assets/templates/main.tex` 加上类文件 `laTeX-模板-GMCMthesis/gmcmthesis.cls`、样式 `gmcm.bst` 与封面图 `figures/logo.pdf`、`figures/title.pdf`；论文内容组织、排版编译与最终交付默认以此为唯一模板来源。step0 初始化项目时，把 `gmcmthesis.cls`、`gmcm.bst`、`figures/logo.pdf`、`figures/title.pdf` 从技能模板目录复制到项目 `论文/` 与 `论文/figures/` 下，主稿以 `assets/templates/main.tex` 为基座落地。论文必须保持 `\documentclass[bwprint]{gmcmthesis}`、封面信息（`\title`、`\baominghao`、`\schoolname`、`\membera/b/c`）、原生 `abstract`、`\keywords{}`、`thebibliography` 参考文献与 `appendices` 附录结构；禁止改写成普通 `article/ctexart`、另造封面或替换官方封面图。GMCMthesis 模板默认不生成目录，禁止 `\tableofcontents`、`\maketoc`、`main.toc` 和“目录”页；封面由 `\maketitle` 生成属模板规定动作，不得删除。
...
跨电脑使用时所需的华为杯 LaTeX 模板见 `assets/templates/main.tex` 与 `laTeX-模板-GMCMthesis/`（含 `gmcmthesis.cls`、`gmcm.bst`、封面图与编译脚本）。
