...
% 硬约束：封面由 \maketitle 生成，页面顺序为 封面→摘要→目录→正文（与官方示例
% example.tex 一致，\maketoc 位于摘要之后）；摘要与关键词合计不得超过两页；
% 正文大于 25 页（不少于 26 页，硬约束）、附录不超过 10 页。
% ============================================================
...
% 图/表全篇连续编号：gmcmthesis.cls 默认用 \numberwithin 让图表随 section 重置
% （图1.1、表2.1）。此处只在模板层用 \counterwithout 取消 figure/table 对 section
% 的计数从属，保持 class 可替换可升级；equation 编号保持 class 原有章节编号不变。
...
% ============================================================
% 摘要与关键词
% 硬约束：摘要与关键词合计不得超过两页（官方允许），摘要有效文字不少于 800 字，
% 建议 850–1050 字；每次改写后必须编译确认 abstract:start 到 abstract:end
% 的 aux 页码跨度不超过 2，禁止缩字号、压行距或负间距规避门禁。
...
%生成目录（GMCMthesis 模板原生 \maketoc，官方示例 example.tex 同款：
%封面→摘要→目录→正文，\maketoc 位于摘要之后，不得删除或前移）
\maketoc
\clearpage

% ============================================================
% 正文：硬约束大于 25 页（不少于 26 页）；不足时只能深化模型建立与求解
% 主结构：一、问题重述；二、模型假设与符号说明；随后按实际问数依次为
% “问题一至问题 N 模型建立与求解”；最后“模型总结与评价”。
% 禁止新增主章节；目录只能由模板原生 \maketoc 生成。
% ============================================================
...
\subsection{符号说明}
% ------------------------------------------------------------------
% 符号表固定使用非浮动排版（禁止 \begin{table} 浮动容器）：
% 1) 短表（可在当前页放下）使用下方 tabularx，紧随标题，不加 caption、不编号；
% 2) 符号内容实际超过一页时，删除上方 tabularx 块，改用下方 longtable 版本：
%    自动跨页并重复表头，同样不加 caption；进入前保存 table 计数、结束后恢复，
%    保证符号表不占用“表1”等正式结果表编号。
% ------------------------------------------------------------------
\renewcommand\arraystretch{1.2}
\newcolumntype{L}{>{\quad}X}
\newcolumntype{P}[1]{>{\centering\arraybackslash}p{#1}}
\noindent\begin{tabularx}{0.9\textwidth}{|P{1cm}|L|}
  \hline
  符号    &   \quad 意义 \\
  \hline
  $ a $  &  符号1的意义    \\
...
% —— 长表版本（符号内容超过一页时替换上方 tabularx 块；按实际编译结果选择分支）——
% \newcounter{huaweisymbolsavetable}
% \setcounter{huaweisymbolsavetable}{\value{table}}
% longtable 不支持上方基于 X 的 L 列；显式分配剩余列宽并保持总宽 0.9\textwidth。
% \begin{longtable}{|P{1cm}|>{\raggedright\arraybackslash}p{\dimexpr0.9\textwidth-1cm-4\tabcolsep-3\arrayrulewidth\relax}|}
%   \hline
%   符号    &   \quad 意义 \\
%   \hline
%   \endfirsthead
...
% 结构（参照官方示例 example.tex 与优秀论文惯例）：数据附录在前——放正文
% 放不下的大表、大规模矩阵与完整数值结果，正文用“限于篇幅，XX 见附录X”引用；
% 代码附录在后——主要实验代码统一放“附录代码文件”一节。
% ============================================================
\clearpage
\label{appendix:start}
\begin{appendices}

% 数据附录：按实际问数增删，标题替换为具体问题（如“问题二附录”）
\section{问题X附录}

在此处放该问正文放不下的实验结果、大规模数据表或矩阵（大概率场景，按题目要求组织）；正文相应位置用“限于篇幅，完整结果见附录 A”引用。
...
在此处放另一问的补充图表、完整数值结果或正文省略的推导细节；无对应内容时整节删除。

% 附录代码统一使用 scripts/prepare_appendix_code.py 生成并验证的净化副本（本行为注释，不进正文）。
\par\noindent\hspace{2em}支撑材料文件目录（以下均为模板占位，只填代码名称和重要 Excel 结果名称，不填路径；纯文件名可保留 .py、.xlsx 后缀）：\par
...
附录代码统一使用 GMCMthesis 模板原生 Python 环境内联净化副本（示例占位，实际替换为全量代码）。

\begin{Python}{appendix\_code\_1.py}
def placeholder():
    return None
\end{Python}
