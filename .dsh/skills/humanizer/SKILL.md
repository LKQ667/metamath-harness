...
本 Skill 只能由用户手动调用。若请求包含 `dsh.mathmodel.request/v1`，其 `skill` 必须为 `humanizer`，直接消费卡片锁定的 `source_path`、`scope`、`sensitivity`、`rewrite_strength`、`protect_formulas`、`protect_citations`、`output_mode`、`target_section` 和 `userNotes`，不得重复询问。

数学建模或学术论文默认先逐段扫描，再只列出由多个具体模式共同支撑的高风险段落。单个标点、单个连接词、正式学术词汇、语法工整或“像机器”这种主观感受都不足以判高风险。`sensitivity` 只调整报告阈值，不得转换成作者身份概率、AIGC 百分比、查重率或任何平台检测分数；本 Skill 不承诺降低检测率，也不提供规避检测保证。

`output_mode=只报告` 或 `rewrite_strength=不改写` 时不得改写正文。只有 `局部修订稿` 或 `报告+局部修订稿` 才改写已定位段落，禁止无差别重写整篇；即使选择“深度”，范围也只扩展到该段与必要的相邻衔接句。每条报告给出位置、命中模式、原文短片段、误判可能和建议处理方式。

论文处理始终保护事实、数值、结论边界、专业术语、变量和单位；`protect_formulas=true` 时公式及编号逐字保持，`protect_citations=true` 时引文命令、引文位置和参考文献条目逐字保持。学术论文不应用 PERSONALITY AND SOUL 中的博客式意见、幽默、刻意凌乱或虚构细节；本入口契约优先于下文通用示例中的文体建议和全局标点替换规则。
...
1. Read the input carefully, locate pattern clusters, and distinguish high-risk passages from isolated false positives.
