#!/usr/bin/env python3
"""Install a skill from a GitHub repo path into $DSH_HOME/skills."""
...
def _dsh_home() -> str:
    return os.environ.get("DSH_HOME", os.path.expanduser("~/.dsh"))


def _tmp_root() -> str:
    base = os.path.join(tempfile.gettempdir(), "deepseek-harness")
    os.makedirs(base, exist_ok=True)
    return base


def _request(url: str) -> bytes:
    return github_request(url, "deepseek-harness-skill-install")
...
def _default_dest() -> str:
    return os.path.join(_dsh_home(), "skills")
