#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Check that the paper uses the built-in math-paper-cn template style."""
...
def main() -> int:
    parser = project_arg("检查 main.tex 是否偏离 math-paper-cn 内置模板。")
    ...
    if "\\tableofcontents" in text:
        errors.append("国赛论文默认不生成目录，`main.tex` 中禁止出现 `\\tableofcontents`。")
