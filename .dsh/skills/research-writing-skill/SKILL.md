...
本 Skill 只能由用户手动调用。若请求包含 `dsh.mathmodel.request/v1`，其 `skill` 必须为 `research-writing-skill`，直接消费卡片锁定的 `source_path`、`section`、`polishing_goal`、`revision_strength`、`protect_terms`、`protect_formulas`、`protect_citations`、`output_format`、`target_range` 和 `userNotes`，不得重复询问；路径相对当前工作区解析，只处理指定章节或范围。

轻度校对只修正错字、语法、标点和明显术语不一致；中度润色可重组句子与段落衔接但不改变论点；深度重构可调整论证顺序，但必须列出结构变化、保留原结论边界，缺证据处只能标记待补，不能补写为事实。相应保护开关为真时，术语、公式/符号、引用命令与参考文献必须逐字或语义等价保留。严格按 `output_format` 输出修订稿、修改说明或对照表。
...
1. Use locked card values first; only when no structured request exists, clarify target: journal/conference/thesis/course report, language, length, audience, and required format.
