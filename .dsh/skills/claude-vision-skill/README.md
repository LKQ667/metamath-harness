...
> DeepSeek Harness 用户：请通过 mathmodel 卡片把 Key 安全保存到 DSH 凭据服务，并使用原生 `vision_analyze` 工具。不要把 Key 写进源码、Prompt 或 Skill `.env`。本目录旧 `.env` 不会被读取、移动或删除；验证新配置后是否清理由用户自行决定。下文是上游独立脚本的兼容说明，不适用于 DSH。
...
2. 由用户在启动进程环境中显式设置 `DASHSCOPE_API_KEY`；可选设置 `VISION_MODELS` 和 `DASHSCOPE_BASE_URL`，不要把 Key 写入源码
...
1. 把 `vision.js` 拷到项目里
2. 在启动进程环境中设置 `DASHSCOPE_API_KEY`；按需设置模型名和 API 地址，不要把 Key 写入源码或 `.env`
