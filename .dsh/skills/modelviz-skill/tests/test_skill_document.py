...
def test_skill_frontmatter_and_no_absolute_paths():
    ...
    # description 必须是「仅手动触发」的中文说明：明确点名 $modelviz-skill 才使用，
    # 其余绘图请求一律不自动触发。这里同时守住触发标记与禁用措辞，避免描述被改回
    # 宽泛的自动触发语义。
    description = metadata["description"]
    assert "$modelviz-skill" in description
    assert "仅手动触发" in description
    assert "禁止自动触发" in description

    assert not re.search(r"[A-Za-z]:\\", text)

    # 旧推荐流水线只允许作为「已知遗留」出现在 Current Limitations 章节，
    # 不能出现在主流程说明里，避免被误当成当前推荐的选图入口。
    limitations_heading = "## Current Limitations"
    assert limitations_heading in text
    main_body, _, _ = text.partition(limitations_heading)
    assert "run_template_matching_pipeline" not in main_body
    assert "template_recommendations.json" not in main_body
