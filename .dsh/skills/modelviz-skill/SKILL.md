---
name: modelviz-skill
description: "仅手动触发的数学建模/科研可视化绘图 Skill（ModelViz）。只有当用户明确点名 `$modelviz-skill`（例如“$modelviz-skill 用我的 data.csv 画……”）时才使用；即使用户提出绘图、出图、可视化、论文配图、图表美化等相关请求，也禁止自动触发。功能：将用户自有的 CSV/XLSX/XLS 数据与自然语言绘图需求，通过内置模板目录选图、适配 Python 模板（不修改原模板）、执行绘图、技术与视觉双重质检并按需修复，产出出版级科研图表（PNG/SVG/PDF + 可复现代码）。"
---
...
- There is no `pyproject.toml` and no `scripts/` entry point; call the Python service entry points directly. `README.md` is a human-facing overview only and is not part of the execution path.
