#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""检查 APMCM 中文赛论文的机械连接词、套话聚集和括号密度。"""
...
def prose_scope(text: str) -> tuple[str, list[str]]:
    abstract = text.find("{\\sectiontitlefont 摘要}")
    bibliography = text.find("\\begin{thebibliography}", abstract + 1)
    if abstract == -1 or bibliography == -1:
        return "", ["无法定位摘要或参考文献，不能确定论文可见论述范围。"]
    return text[abstract:bibliography], []
...
def main() -> int:
    parser = project_arg("检查 APMCM 中文赛论文自然文风、机械套话和括号密度")
