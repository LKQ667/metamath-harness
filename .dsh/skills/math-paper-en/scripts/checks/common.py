#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""Shared helpers for math-paper-en deliverable checks."""
...
SUMMARY_TITLE_MARKER = "{\\sectiontitlefont Summary}"
CATALOG_MARKER = "Supporting Material Catalog"
APPENDIX_CODE_MARKER = "\\section{Code}"
...
def project_arg(description: str) -> argparse.ArgumentParser:
    ...
    parser.add_argument("--project", required=True, help="数学建模项目根目录（math-paper-en 项目契约与 math-paper-cn 一致）")
...
def paper_body_region(text: str) -> tuple[str, list[str]]:
    ...
    title = text.find("{\\titlefont", start)
    abstract = text.find(SUMMARY_TITLE_MARKER, start)
    ...
    catalog = text.find(CATALOG_MARKER, appendix)
    appendix_code = text.find(APPENDIX_CODE_MARKER, appendix)

    if document == -1 or not (document < start):
        errors.append("`body:start` 必须位于 `\\begin{document}` 之后。")
    if title == -1 or not (start < title < end):
        errors.append("论文标题必须位于 `body:start` 之后和 `body:end` 之前。")
    if abstract == -1 or not (start < abstract < end):
        errors.append("Summary Sheet 必须位于 `body:start` 与 `body:end` 之间。")
    ...
    if catalog == -1 or not (appendix < catalog):
        errors.append("Supporting Material Catalog 必须位于 `appendix:start` 之后。")
    if appendix_code == -1 or not (appendix < appendix_code):
        errors.append("附录代码章节必须位于 `appendix:start` 之后。")
