...
DRAWIO_QA = ("static_check_ok", "cli_export_ok", "content_ok", "label_text_ok", "layout_ok", "edge_routing_ok", "node_overlap_ok", "text_fit_ok", "grayscale_ok", "single_column_ok", "double_column_ok", "paper_insert_ok")
AI_QA = ("content_consistency_ok", "label_text_ok", "symbol_formula_ok", "crop_ok", "clarity_ok", "single_column_ok", "double_column_ok", "paper_insert_ok")
...
def check_drawio(project: Path, items: list[dict], prompts: list[Path], errors: list[str]) -> None:
    ...
    if proof:
        ...
        required_proof = proof.get("version_ok") is True and proof.get("source_ok") is True and proof.get("label_text_ok") is True
