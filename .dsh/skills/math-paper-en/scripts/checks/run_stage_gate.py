...
def launcher_source(default_args: list[str]) -> str:
    ...
    return (
        "#!/usr/bin/env python\n"
        "# -*- coding: utf-8 -*-\n"
        '"""由 math-paper-en 自动生成；不要改写或复制门禁逻辑。"""\n'
        "import hashlib\n"
        "from pathlib import Path\n"
        "import os\n"
        "import subprocess\n"
        ...
        f"PRIMARY_RUNNER = {str(runner_path)!r}\n"
        "candidates = [Path(PRIMARY_RUNNER)]\n"
        "for root in (os.environ.get('DSH_HOME'), Path.home() / '.dsh', Path.home() / '.codex', Path.home() / '.trae-cn'):\n"
        "    if root:\n"
        "        candidates.append(Path(root) / 'skills' / 'math-paper-en' / 'scripts' / 'checks' / 'run_stage_gate.py')\n"
        ...
        "if RUNNER is None:\n"
        "    raise SystemExit('门禁源缺失或版本已变化，请从当前 math-paper-en Skill 重新运行 --init。')\n"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="运行 math-paper-en 逐阶段硬门禁")
