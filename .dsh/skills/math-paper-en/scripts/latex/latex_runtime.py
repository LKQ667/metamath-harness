...
def choose_runtime_root(
    ...
    for drive in ordered:
        candidate = drive / "math-paper-en-runtime" / f"texlive-{release}"
        try:
...
def download(url: str, output: Path, timeout: int = 120) -> None:
    request = urllib.request.Request(url, headers={"User-Agent": "math-paper-en/1"})
...
def run_installer(archive: Path, root: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="installer-", dir=root.parent) as tmp:
        ...
        profile = temp / "math-paper-en.profile"
...
\end{document}
"""
    with tempfile.TemporaryDirectory(prefix="math-paper-en-smoke-") as tmp:
        root = Path(tmp)
        tex = root / "smoke.tex"
...
def main() -> int:
    parser = argparse.ArgumentParser(description="math-paper-en Windows LaTeX 运行时管理器")
