...
class LatexRuntimeTests(unittest.TestCase):
    def test_ascii_path(self) -> None:
        self.assertTrue(latex_runtime.is_ascii_path(Path(r"F:\math-paper-en-runtime")))
    ...
    def test_render_profile_uses_current_texlive_keys(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            latex_runtime.render_profile(Path(r"F:\math-paper-en-runtime\texlive-2026"), output)
            text = output.read_text(encoding="utf-8")
            self.assertIn("binary_windows 1", text)
            self.assertIn("tlpdbopt_install_docfiles 0", text)
            self.assertIn("TEXMFSYSVAR F:/math-paper-en-runtime/texlive-2026/texmf-var", text)
