...
# 跨问图型多样性：每一问的主图必须落在互不相同的 chart_family 上。
# 该列表是把「任务语义 -> 图型」的备选池按信息密度排序后的全局兜底序列。
DIVERSITY_FALLBACK = (
    "tornado",
    "interval_plot",
    "lollipop",
...
HEATMAP_FAMILIES = ("heatmap", "grid_heatmap", "sobol_heatmap")
BAR_FAMILIES = ("vertical_bar", "bar", "barh", "column", "stacked_bar")


def family_quota_ok(family: str, used_families) -> bool:
    """热力图与柱状图在全文最多各出现一次，其余图型同样不允许跨问重复。"""
    used = {str(item).strip().lower() for item in used_families or ()}
    key = str(family).strip().lower()
    if key in used:
        return False
    if key in HEATMAP_FAMILIES and used & set(HEATMAP_FAMILIES):
        return False
    if key in BAR_FAMILIES and used & set(BAR_FAMILIES):
        return False
    return True


def enforce_family_diversity(pick: dict, used_families) -> dict:
    """在候选池里挑选尚未被其它小问占用的图型，必要时升级到全局兜底序列。"""
    used = [str(item).strip().lower() for item in used_families or ()]
    candidates = [pick.get("family"), *(pick.get("preferred") or [])]
    for candidate in candidates:
        if candidate and family_quota_ok(candidate, used):
            pick["family"] = candidate
            pick["diversity_ok"] = True
            pick["used_families"] = list(used)
            return pick
    for candidate in DIVERSITY_FALLBACK:
        if family_quota_ok(candidate, used):
            pick["family"] = candidate
            pick["diversity_forced"] = True
            pick["diversity_ok"] = True
            pick["used_families"] = list(used)
            pick["reason"] = str(pick.get("reason", "")).rstrip() + f" 首选图型已被其它小问占用，按跨问多样性强制改用 {candidate}。"
            return pick
    pick["family"] = None
    pick["diversity_ok"] = False
    pick["used_families"] = list(used)
    pick["reason"] = str(pick.get("reason", "")).rstrip() + " 全文可用图型已耗尽，必须重新设计图型分配而不能复用同一图型。"
    return pick

def _pick_chart_family_shape(
    task_type: str,
    data_semantics: str = "",
    compare_goal: str = "",
...
def choose_chart_family(
    ...
    used_families 传入「其它小问已经占用的 chart_family」集合，返回的 family 保证
    与它们不重复；热力图与柱状图在全文最多各一张，非数据流程图不参与配额。
    """
    pick = _pick_chart_family_shape(task_type, data_semantics, compare_goal, need_uncertainty)
    if pick.get("non_data"):
        pick["diversity_ok"] = True
        pick["used_families"] = [str(item).strip().lower() for item in used_families or ()]
        return pick
    return enforce_family_diversity(pick, used_families)
...
def run_py_nature_qa(fig_path_or_fig_object, profile: str = "competition_cn") -> QAResult:
    ...
    if hasattr(fig_path_or_fig_object, "savefig"):
        checks = {
            ...
            "label_text_ok": _cn_font_configured() if profile == "competition_cn" else True,
    ...
    checks["label_text_ok"] = checks["font_profile_ok"] if profile == "competition_cn" else True
    ...
    required = [
        ...
        "svg_color_count_reasonable", "label_text_ok",
    ]
    if profile == "research":
