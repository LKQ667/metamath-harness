...
def find_drawio() -> Path | None:
    runtime_root = os.environ.get("DSH_RUNTIME_ROOT") or os.environ.get("MATH_PAPER_EN_RUNTIME")
...
def verify_cli(executable: Path, work_dir: Path) -> dict:
    ...
    if svg_path.exists():
        ...
        result["label_text_ok"] = "中文字体验证" in svg_text or "Microsoft YaHei" in svg_text
    result["ok"] = all((result["version_ok"], result["source_ok"], result["label_text_ok"], *result["exports"].values()))
    return result
