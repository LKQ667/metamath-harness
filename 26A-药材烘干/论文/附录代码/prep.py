...
def describe_series(series: pd.Series) -> dict:
    ...
    return {'数量': int(values.size), '最小值': round(float(values.min()), 6), '最大值': round(float(values.max()), 6), '均值': round(float(values.mean()), 6), '标准差': round(float(values.std(ddof=1)), 6), '严格递增': bool(np.all(diffs > 0)), '单调不增': bool(np.all(diffs <= 0))}
...
def build_summary(ambient: pd.DataFrame, radius: pd.DataFrame) -> dict:
    ...
    return {'附件1': {'记录数': int(ambient.shape[0]), '时间范围_s': [float(ambient_time.min()), float(ambient_time.max())], '时间步长_s': step_ambient, '缺失值': int(ambient.isna().sum().sum()), '重复时间点': int(ambient['时间'].duplicated().sum()), '温度': describe_series(ambient['温度']), '水分浓度': describe_series(ambient['水分浓度']), '温度与水分浓度相关系数': round(correlation, 6)}, '附件2': {'记录数': int(radius.shape[0]), '时间范围_s': [float(radius_time.min()), float(radius_time.max())], '时间步长_s': step_radius, '缺失值': int(radius.isna().sum().sum()), '重复时间点': int(radius['时间'].duplicated().sum()), '半径': describe_series(radius['半径']), '首末半径_cm': [round(float(radius['半径'].iloc[0]), 6), round(float(radius['半径'].iloc[-1]), 6)], '收缩幅度_cm': round(float(radius['半径'].iloc[0] - radius['半径'].iloc[-1]), 6)}, '结论': ['两份附件均无缺失与重复，时间严格递增，可直接作为边界工况与几何输入。', '烘房温度与水分浓度同向上升且高度正相关，说明加热过程伴随水分蒸发进入烘房。', '药材半径单调收缩并在后段趋于平稳，问题 4 的移动边界需用保形插值重建连续曲线。']}
...
def main() -> None:
    ...
    (DERIVED_DIR / 'eda_summary.json').write_text(json.dumps(summary, ensure_ascii=False, indent=2), encoding='utf-8')
