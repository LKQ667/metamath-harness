...
def solve_cylinder(props: Properties, radius: float, t_end: float, dt: float, n_cells: int, t_initial: float, c_initial: float, t_air: Callable[[float], float], c_air: Callable[[float], float], h: float, h_m: float, record_every: int=1, stop_moisture: float | None=None, max_steps: int | None=None, keep_index: np.ndarray | None=None) -> SolveResult:
    dr = radius / n_cells
    ...
    for step in range(1, total + 1):
        ...
        if stopped:
            stop_index = step
            break
    return SolveResult(times=np.array(times), radii=r[keep], temperature=np.array(temp_hist), moisture=np.array(moist_hist), meta={'dr_cm': dr * 100.0, 'dt_s': dt, 'steps': len(times) - 1, 'stop_index': stop_index, 'stop_time_s': times[-1] if stop_index is not None else None, 'radius_cm': radius * 100.0})

def solve_shrinking(props: Properties, radius_of: Callable[[float], float], rate_of: Callable[[float], float], t_end: float, dt: float, n_cells: int, t_initial: float, c_initial: float, t_air: Callable[[float], float], c_air: Callable[[float], float], h: float, h_m: float, record_every: int=1, stop_moisture: float | None=None, max_steps: int | None=None) -> SolveResult:
    dr = 1.0 / n_cells
    ...
    for step in range(1, total + 1):
        ...
        diff = props.d(moisture, temperature)
        alpha = k / (rho * cp)
        moisture = _shrink_step(moisture, diff, xi, xi_face, weight, dr, radius, adv, h_m / radius, float(c_air(t_now)), dt)
        temperature = _shrink_step(temperature, alpha, xi, xi_face, weight, dr, radius, adv, h / (rho[-1] * cp[-1] * radius), float(t_air(t_now)), dt)
        stopped = stop_moisture is not None and float(moisture.max()) < stop_moisture
        if stopped or step % max(int(record_every), 1) == 0:
            times.append(t_now)
        ...
        if stopped:
            stop_index = step
            break
    return SolveResult(times=np.array(times), radii=xi * radius_hist[-1], temperature=np.array(temp_hist), moisture=np.array(moist_hist), meta={'dt_s': dt, 'steps': len(times) - 1, 'stop_index': stop_index, 'stop_time_s': times[-1] if stop_index is not None else None, 'radius_hist_cm': [value * 100.0 for value in radius_hist]})
...
def properties_from_appendix2() -> Properties:
    ...
    def d(c, t):
        return 7e-09 * np.exp(-0.89 / np.maximum(c, 1e-06))
...
def properties_from_appendix3() -> Properties:
    ...
    def d(c, t):
        tk = t + 273.15
        return 0.0024 * np.exp(-0.45 / np.maximum(c, 1e-06)) * np.exp(-3850.0 / tk)
    return Properties(rho=rho, cp=cp, k=k, d=d)
...
def properties_from_appendix4() -> Properties:
    ...
    def k(c, t):
        return 0.12 + 0.2 * c / (c + 1.0)

    def d(c, t):
        tk = t + 273.15
        return 0.00042 * np.exp(-0.3 / np.maximum(c, 1e-06)) * np.exp(-3850.0 / tk)
    return Properties(rho=rho, cp=cp, k=k, d=d)
