"""问题 2：整个烘干过程（预热平衡 + 恒温干燥）3 h 内的温度与水分浓度场。

输出：
  results/result2.xlsx        —— 3 h 内每隔 1 s、径向每隔 0.1 cm 的完整结果
  results/q2_summary.json     —— 本问关键数值（供唯一结果源聚合）
"""
...
def main() -> None:
    ...
    summary = {
        ...
        "result_surface_temperature_3h_C": round(float(result.temperature[last, -1]), 4),
        "result_center_temperature_3h_C": round(float(result.temperature[last, 0]), 4),
        "result_surface_moisture_3h": round(float(result.moisture[last, -1]), 4),
        "result_center_moisture_3h": round(float(result.moisture[last, 0]), 4),
        "result_surface_moisture_drop_3h": round(float(C0 - result.moisture[last, -1]), 4),
        "result_center_moisture_drop_3h": round(float(C0 - result.moisture[last, 0]), 4),
        "meta": {
            "dt_s": DT_S,
            "n_cells": N_CELLS,
            "radius_cm": 2.0,
