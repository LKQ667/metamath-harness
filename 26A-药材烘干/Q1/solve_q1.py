"""问题 1：预热平衡阶段温度与水分浓度场求解。

输出：
  results/result1.xlsx        —— 1800 s 内每隔 1 s、径向每隔 0.1 cm 的完整结果
  results/q1_summary.json     —— 本问关键数值（供唯一结果源聚合）
"""
...
def main() -> None:
    ...
    with pd.ExcelWriter(RESULTS_DIR / "result1.xlsx", engine="openpyxl") as writer:
        pd.DataFrame(np.column_stack([seconds, temperature]), columns=header).to_excel(
            writer, sheet_name="温度", index=False
        )
        pd.DataFrame(np.column_stack([seconds, moisture]), columns=header).to_excel(
            writer, sheet_name="水分浓度", index=False
        )
    ...
    changed = np.where(result.moisture[last] < C0 - 1e-3)[0]
    penetration_cm = 0.0 if changed.size == 0 else round(float(2.0 - radii_cm[changed.min()]), 4)
    summary = {
        "question": 1,
        "table_columns_cm": table_r,
        "temperature_table": rows_t,
        "moisture_table": rows_c,
        "result_surface_temperature_1800s_C": round(float(result.temperature[last, -1]), 4),
        "result_center_temperature_1800s_C": round(float(result.temperature[last, 0]), 4),
        "result_surface_moisture_1800s": round(float(result.moisture[last, -1]), 4),
        "result_center_moisture_1800s": round(float(result.moisture[last, 0]), 4),
        "result_surface_moisture_drop_1800s": round(float(C0 - result.moisture[last, -1]), 4),
        "result_penetration_depth_1800s_cm": penetration_cm,
        "meta": {
            "dt_s": DT_S,
            "n_cells": N_CELLS,
            "radius_cm": 2.0,
    ...
    (RESULTS_DIR / "q1_summary.json").write_text(
