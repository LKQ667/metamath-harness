...
移动边界在固定坐标 xi = r / R(t) 上求解，R(t) 由附件 2 的实测半径经保形插值重建，
t 超过附件 2 覆盖范围后按最后实测值保持恒定，不做外推。

输出：
  results/result4.xlsx        —— 每隔 60 s 的水分浓度（固定距离列 + 药材表面列）
  results/q4_summary.json     —— 本问关键数值（供唯一结果源聚合）
"""
...
from drying_model import properties_from_appendix4, solve_shrinking  # noqa: E402
...
MAX_DAYS = 30.0
RECORD_EVERY = 6
COLUMN_R_CM = np.round(np.arange(0.0, 1.1 + 1e-9, 0.1), 1)
...
def main() -> None:
    ...
    pd.DataFrame(np.column_stack([seconds, table]), columns=header).to_excel(
        RESULTS_DIR / "result4.xlsx", sheet_name="Sheet1", index=False
    )
    ...
    summary = {
        "question": 4,
        "table_columns_cm": [float(v) for v in COLUMN_R_CM] + ["药材表面"],
        "moisture_table": rows_c,
        "result_drying_time_h": drying_time_h,
        "result_drying_time_days": round(drying_time_h / 24.0, 4),
        "result_final_radius_cm": final_radius_cm,
        "result_initial_radius_cm": 2.0,
        "result_shrinkage_ratio": round(1.0 - final_radius_cm / 2.0, 4),
        "result_surface_moisture_at_end": round(float(table[final_step, -1]), 4),
        "result_center_moisture_at_end": round(float(table[final_step, 0]), 4),
        "result_moisture_threshold": TARGET,
        "meta": {
            "dt_s": DT_S,
            "n_cells": N_CELLS,
            "rows": int(seconds.size),
            "radius_source": "附件2 保形插值，超出覆盖范围按末值保持",
            "radius_covered_h": round(float(time[-1]) / 3600.0, 4),
            "radius_last_cm": round(float(radius_cm[-1]), 4),
        },
    }
    (RESULTS_DIR / "q4_summary.json").write_text(
