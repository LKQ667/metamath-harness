...
def _scale_audio_columns(audio: np.ndarray) -> np.ndarray:
    ...
    audio[..., AUDIO_SCALE_DIM] = np.sign(audio[..., AUDIO_SCALE_DIM]) * np.log1p(
        np.abs(audio[..., AUDIO_SCALE_DIM])
    )
    return audio
...
def quantile_reference(aligned: dict) -> dict:
    """按训练集每个特征维度计算分位函数，供分布不同版本的特征做秩映射。"""
    reference = {}
    ...
    for modality in MODALITIES:
        ...
        if modality == "audio":
            train = _scale_audio_columns(train)
        flat = train.reshape(-1, train.shape[-1])
        reference[modality] = {"grid": grid,
                               "values": np.quantile(flat, grid, axis=0).astype(np.float64)}
    return reference


def rank_map(array: np.ndarray, modality: str, reference: dict, stats: dict) -> np.ndarray:
    """把分布不同版本的模态特征按秩映射到训练集每个维度的分位分布，再标准化。

    映射保持同一样本内各维度的相对次序不变，只校正整体分布偏移；缺失位置（全零）保持全零。"""
...
def load_a4() -> dict:
    ...
    return {
        ...
        "audio": data["audio"],
        "vision": data["vision"],
        "text_bert": data["text_bert"],
        "ids": data["ids"],
    }


def a3_missing_plan() -> list[dict]:
    """把附件 3 每个样本的缺失区间转成可复用的缺失施加计划。"""
    import json
...
def sample_missing_segments(rng: np.random.Generator, seq_len: int, ratio: float,
                            max_segments: int = 3, min_len: int = 1) -> list[tuple[int, int]]:
...
def metrics_classification(pred: np.ndarray, target: np.ndarray) -> dict:
    ...
    return {
        ...
        "weighted_f1": float(np.average(f1_values, weights=[int((target == label).sum()) for label in labels]))
        if labels and int((target == labels[0]).sum()) >= 0 else 0.0,
    }
