...
class ReliabilityGate(nn.Module):
    ...
    def forward(self, sequence: torch.Tensor, mask: torch.Tensor, modality_index: int) -> torch.Tensor:
        ...
        embed = self.embedding(
            torch.full((batch, steps), modality_index, dtype=torch.long, device=sequence.device)
        )
...
class RobustFusion(nn.Module):
    """缺失感知的可信度门控 + 跨模态注意力的鲁棒融合模型。

    在时序门控融合之外，另加一条跨模态交互支路：把各模态的时序摘要与可用性摘要拼成一个
    跨模态描述向量，用于表达"哪些模态一致、哪些模态被削弱"这类全局结构。缺失条件下
    时序局部信息减少，这条全局支路提供互补证据。"""

    def __init__(self, hidden: int = 192, n_modalities: int = 3, dropout: float = 0.15,
                 use_gate: bool = True, use_attention: bool = True):
        super().__init__()
        self.hidden = hidden
        ...
        self.attention = (
            nn.MultiheadAttention(hidden, num_heads=4, dropout=dropout, batch_first=True)
        ...
        self.summary = nn.Sequential(
            nn.Linear(hidden * n_modalities * 2 + n_modalities, hidden),
            nn.GELU(),
            nn.Dropout(dropout),
            nn.Linear(hidden, hidden),
        )
    ...
    def forward(self, sequences: list[torch.Tensor], masks: list[torch.Tensor]) -> dict:
        ...
        if self.attention is not None:
            key_padding = (weight_sum.squeeze(-1) <= 1e-6)
        ...
        valid = (weight_sum.squeeze(-1) > 1e-6)
        ...
        return {
            ...
            "regression": self.reg(pooled).squeeze(-1),
            "pooled": pooled,
...
class MeanFusion(nn.Module):
    ...
    def forward(self, sequences: list[torch.Tensor], masks: list[torch.Tensor]) -> dict:
        ...
        fused = (stacked * stacked_mask.unsqueeze(-1)).sum(dim=2) / mass.clamp_min(1e-6)
        ...
        modality_mass = modality_mass / modality_mass.sum(dim=1, keepdim=True).clamp_min(1e-6)
        return {
            "logits": self.cls(pooled),
            "regression": self.reg(pooled).squeeze(-1),
            "pooled": pooled,
            ...
            "gate_values": [stacked_mask[:, :, i] for i in range(stacked_mask.shape[2])],
            "attention_map": None,
            "fused": fused,
        }
...
class TensorFusion(nn.Module):
    ...
    def forward(self, sequences: list[torch.Tensor], masks: list[torch.Tensor]) -> dict:
        ...
        fused = self.norm(fused + self.drop(self.temporal(fused)))
        score = self.pool_query(fused).squeeze(-1).masked_fill(~valid, -1e9)
        weight = torch.softmax(score, dim=1) * valid.float()
        weight = weight / weight.sum(dim=1, keepdim=True).clamp_min(1e-6)
        pooled = (fused * weight.unsqueeze(-1)).sum(dim=1)
        stacked_mask = torch.stack(masks, dim=2)
...
class LowRankFusion(nn.Module):
    ...
    def forward(self, sequences: list[torch.Tensor], masks: list[torch.Tensor]) -> dict:
        projected = [torch.einsum("bth,hr->btr", sequence, factor)
                     for sequence, factor in zip(sequences, self.factors)]
        stacked_mask = torch.stack(masks, dim=2)
        mass = stacked_mask.sum(dim=1)
        gate = mass / mass.sum(dim=1, keepdim=True).clamp_min(1e-6)
        ...
        pool_weight = pool_weight / pool_weight.sum(dim=1, keepdim=True).clamp_min(1e-6)
...
MODEL_NAMES = (
    "robust_full", "robust_no_gate", "robust_no_attention",
    "mean_fusion", "tensor_fusion", "lowrank_fusion",
    "single_text", "single_audio", "single_vision",
)
