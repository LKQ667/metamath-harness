...
class InterpretableFusion(nn.Module):
    """可解释融合：模态级注意力、位置级重要性与逐位置模态贡献三层证据。"""
    ...
    def __init__(self, hidden: int = 192, n_modalities: int = 3, dropout: float = 0.15):
        ...
        self.fuse = nn.Sequential(
            nn.Linear(hidden * n_modalities, hidden),
            nn.GELU(),
            nn.Dropout(dropout),
    ...
    def forward(self, sequences: list[torch.Tensor], masks: list[torch.Tensor]) -> dict:
        ...
        for index, item in enumerate(projected):
            mask = masks[index]
            score = self.modality_query(item).squeeze(-1).masked_fill(mask <= 0, -1e9)
        ...
        valid = weight_sum > 0
        compat = torch.stack(
            [torch.tanh(self.compat[i](projected[i])) for i in range(self.n_modalities)], dim=2
        )
        fused = self.fuse(compat.flatten(2))
        fused = self.norm(fused)
        position_score = self.position_query(fused).squeeze(-1)
        position_score = position_score.masked_fill(~valid, -1e9)
        ...
        for index in range(self.n_modalities):
            mask = masks[index]
            score = self.modality_query(projected[index]).squeeze(-1).masked_fill(mask <= 0, -1e9)
            weight = torch.softmax(score, dim=1) * mask
            weight = weight / weight.sum(dim=1, keepdim=True).clamp_min(1e-6)
            contribution[:, index, :] = alpha[:, index].unsqueeze(1) * weight * mask
        return {
            "logits": self.cls(pooled_final),
            "regression": self.reg(pooled_final).squeeze(-1),
            "pooled": pooled_final,
