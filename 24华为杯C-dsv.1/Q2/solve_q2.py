"""问题二：销售量、亩产量、成本与价格不确定性下的最优种植方案与风险权衡。"""
...
WHEAT_MAIZE = (6, 7)
CORNERS = [
    ("基准情形", {"grain": 0.075, "other": 0.0, "yld": 0.0, "cost": 0.05, "veg": 0.05, "fung": 0.03}),
    ("粮价需求高增长", {"grain": 0.10, "other": 0.05, "yld": 0.05, "cost": 0.05, "veg": 0.05, "fung": 0.03}),
    ("粮价需求低增长", {"grain": 0.05, "other": -0.05, "yld": -0.05, "cost": 0.05, "veg": 0.05, "fung": 0.03}),
    ("产量与成本双压", {"grain": 0.075, "other": 0.0, "yld": -0.10, "cost": 0.08, "veg": 0.05, "fung": 0.05}),
    ("产量与成本双松", {"grain": 0.075, "other": 0.0, "yld": 0.10, "cost": 0.03, "veg": 0.05, "fung": 0.01}),
    ("蔬菜价格快涨", {"grain": 0.075, "other": 0.05, "yld": 0.0, "cost": 0.05, "veg": 0.07, "fung": 0.01}),
    ("食用菌价格快降", {"grain": 0.075, "other": -0.05, "yld": 0.0, "cost": 0.05, "veg": 0.03, "fung": 0.05}),
    ("综合不利", {"grain": 0.05, "other": -0.05, "yld": -0.10, "cost": 0.08, "veg": 0.03, "fung": 0.05}),
]
if FAST:
    CORNERS = CORNERS[:1]
...
def main():
    ...
    out = {"alpha": ALPHA, "scenario_count": count, "scenarios": [name for name, _ in CORNERS],
           "drivers": {name: driver for name, driver in CORNERS}, "frontier": [], "plans": {}, "single": []}
    ...
    for index, (name, driver) in enumerate(CORNERS):
        ...
        row = {
            ...
            "plan_rows": len(plan),
            "profit_value": round(stats["mean"], 2),
