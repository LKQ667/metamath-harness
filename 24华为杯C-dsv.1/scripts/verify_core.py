...
def run(data, yld, cst, risk, coef, label):
    ...
    return {
        ...
        "objective": round(float(result["objective"] or 0.0), 2),
        "rows": len(rows),
        "area": round(sum(row["area"] for row in rows), 4),
        "audit": {key: audit[key] for key in ("repeat_count", "bean_window_fail", "over_capacity", "undersize_count")},
    }
