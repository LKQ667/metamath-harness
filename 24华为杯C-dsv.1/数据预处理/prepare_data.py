...
def read_plots() -> pd.DataFrame:
    ...
    keep = frame["plot"].str.match(r"^[A-F]\d+$")
...
def read_crops() -> pd.DataFrame:
    ...
    frame["space_list"] = [
        spaces if spaces else fallback.get(kind, [])
        for spaces, kind in zip(frame["space_list"], frame["ctype"])
    ]
    frame["is_bean"] = frame["crop_id"].isin(BEAN_FOOD + BEAN_VEG)
    frame["family"] = frame["ctype"].map(
        lambda value: "粮食（豆类）"
        if "豆类" in value and "粮食" in value
        else "蔬菜（豆类）"
        if "豆类" in value
        else value
    )
...
def main() -> None:
    ...
    base_merge["plot_type"] = base_merge["plot_type"].fillna(
        base_merge["ctype"].map(lambda value: "平旱地" if "粮食" in value else "水浇地")
    )
    ...
    demand = (
        base_merge.groupby(["crop_id", "crop"], as_index=False)
        .agg(
            sales2023_jin=("production_jin", "sum"),
            area2023=("area", "sum"),
            revenue2023_yuan=("production_jin", lambda values: float(values.sum())),
        )
        .sort_values("crop_id")
    )
    revenue = (
        base_merge.assign(revenue=base_merge["production_jin"] * base_merge["price_mid"])
        .groupby(["crop_id", "crop"], as_index=False)
        .agg(revenue2023_yuan=("revenue", "sum"))
    )
    ...
    summary = {
        "地块数量": int(len(plots)),
        "地块总面积亩": round(area_total, 4),
        "地块类型分布": {str(k): int(v) for k, v in plots["plot_type"].value_counts().items()},
        "地块类型面积": {str(k): round(float(v), 4) for k, v in plots.groupby("plot_type")["area"].sum().items()},
        "作物数量": int(len(crops)),
        "作物类型分布": {str(k): int(v) for k, v in crops["family"].value_counts().items()},
        "豆类作物数量": int(crops["is_bean"].sum()),
        "参数记录数": int(len(params)),
        "2023种植记录数": int(len(base)),
        "缺失值统计": {
            "附件1_plots": int(plots.isna().sum().sum()),
            "附件1_crops": int(crops.isna().sum().sum()),
            "附件2_params": int(params.isna().sum().sum()),
            "附件2_base2023": int(base.isna().sum().sum()),
        },
        "重复值统计": {
            "附件1_plots地块名重复": int(plots["plot"].duplicated().sum()),
            "附件2_params组合重复": int(params.duplicated(subset=["crop_id", "plot_type", "season"]).sum()),
        },
        "亩产量区间": [round(float(params["yield_jin"].min()), 4), round(float(params["yield_jin"].max()), 4)],
        "种植成本区间": [round(float(params["cost_yuan"].min()), 4), round(float(params["cost_yuan"].max()), 4)],
        "销售价格区间": [round(float(params["price_low"].min()), 4), round(float(params["price_high"].max()), 4)],
        "2023总产值元": round(float((base_merge["production_jin"] * base_merge["price_mid"]).sum()), 4),
        "2023总成本元": round(float((base_merge["area"] * base_merge["cost_yuan"]).sum()), 4),
        "折旧系数": round(
            float(
                (
                    base_merge["production_jin"].fillna(0) * base_merge["price_mid"].fillna(0)
                    - base_merge["area"] * base_merge["cost_yuan"].fillna(0)
                ).sum()
                / (base_merge["production_jin"].fillna(0) * base_merge["price_mid"].fillna(0)).sum()
            ),
            6,
        ),
        "预期销售量口径": "以 2023 年各作物实际总产量作为 2023 年销售量，问题一假定其保持稳定",
        "类别均价": {
            str(k): round(float(v), 4)
            for k, v in params.groupby("plot_type")["price_mid"].mean().items()
        },
    }
