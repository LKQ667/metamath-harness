...
def plots_all():
    ...
    return [
        {"plot": row["plot"], "kind": row["plot_type"], "area": num(row["area"])}
        for row in rows
    ]
...
def build_params(plots, crops):
    ...
    for row in rows:
        ...
        index[key] = {
            "yield_jin": num(row["yield_jin"]),
            "cost_yuan": num(row["cost_yuan"]),
...
def load_all():
    ...
    return {
        ...
        "up_veg": UP_VEG,
        "gh_veg": GH_VEG,
        "price_by_crop": price_by_crop({"plots": plots, "crops": crops, "params": params, "base2023": base}),
        "cost_by_kind": cost_by_kind({"plots": plots, "crops": crops, "params": params}),
        "yield_by_kind": yield_by_kind({"plots": plots, "crops": crops, "params": params, "base2023": base}),
    }
...
def price_by_crop(data):
    """按 2023 年实际种植面积加权，得到每类作物的名义价格区间。"""
    weight = {}
    for row in data["base2023"]:
