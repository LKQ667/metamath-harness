"""农作物种植策略的混合整数规划模型：决策变量、约束、求解与结果整理。"""
...
def build(cfg):
    ...
    for name in plots:
        ...
        if kind[name] == "水浇地":
            for year in years:
                first = pulp.lpSum(
                    x[(name, cid, "第一季", year)] for cid in data["crops"] if (name, cid, "第一季") in allow
                )
                second = pulp.lpSum(
                    x[(name, cid, "第二季", year)] for cid in data["crops"] if (name, cid, "第二季") in allow
                )
    ...
    for name in plots:
        ...
        for year in years:
            terms = [
                x[(name, cid, season, year)]
                for cid in ed.BEAN
                for season in ed.season_of(kind[name], cid)
                if (name, cid, season, year) in x
            ]
            if not terms:
                continue
    ...
    for crop_id in data["crops"]:
        for year in years:
            terms = [
                yld(crop_id, name, year) * x[(name, crop_id, season, year)]
                for name in plots
                for season in ed.season_of(kind[name], crop_id)
                if (name, crop_id, season, year) in x
            ]
            if not terms:
                continue
    ...
    for year in years:
        ...
        outlay = pulp.lpSum(
            cst(crop_id, name, year) * x[(name, crop_id, season, year)]
            for (name, crop_id, season, year) in x
        )
        if pen > 0:
    ...
    if risk > 0:
        ...
        eta = pulp.LpVariable("eta", lowBound=-1e9)
...
def solve(model, time_limit=600, gap=0.005):
    ...
    return {
        ...
        "profit": {year: (pulp.value(value) or 0.0) for year, value in model["profit"].items()},
        "runtime": model["problem"].solutionTime,
    }
        ...
        rows.append(
            {
                ...
                "crop": data["crops"][crop_id],
                "year": year,
                "produce_jin": round(total, 2),
                "sales_jin": round(sold, 2),
                "surplus_jin": round(max(0.0, total - cap_value), 2),
                "discard_jin": round(max(0.0, loss), 2),
                "revenue_yuan": round(revenue, 2),
                "price_yuan": round(price, 4),
                "cap_jin": round(cap_value, 2),
            }
        )
...
def audit(model):
    ...
    for year in model["years"]:
        for name in model["plots"]:
            ...
            if total > area[name] + 1e-3:
                over.append({"plot": name, "year": year, "area": round(total, 4)})
    small = [
        {"plot": row["plot"], "crop_id": row["crop_id"], "year": row["year"], "area": row["area"]}
        for row in rows
        if row["area"] < 0.3 - 1e-6
    ]
    return {
        "repeat_count": len(repeat),
        "bean_window_fail": len(bean_bad),
        "over_capacity": len(over),
        "undersize_count": len(small),
        "repeat_detail": repeat[:20],
        "bean_detail": bean_bad[:20],
        "over_detail": over[:20],
    }
        ...
        rows.append(
            {
                ...
                "crop": data["crops"][crop_id],
                "year": year,
                "margin_yuan": round(var.value() or 0.0, 2),
                "revenue_yuan": round(model["revenue"][key].value() or 0.0, 2),
                "produce_jin": round(model["produce"][key].value() or 0.0, 2),
            }
        )
    rows.sort(key=lambda item: (item["year"], item["crop_id"]))
    return rows
...
def summary(model, result):
    ...
    return {
        "objective": round(result["objective"] or 0.0, 2),
        "status": result["status"],
        "profit_by_year": profit,
        "revenue_yuan": round(revenue, 2),
        "margin_yuan": round(margin_total, 2),
        "cost_yuan": round(outlay, 2),
        "profit_value_yuan": round(margin_total - outlay, 2) if model["pen"] > 0 else round(revenue - outlay, 2),
        "produce_jin": round(produce_total, 2),
        "sales_jin": round(sold_total, 2),
        "surplus_jin": round(surplus_total, 2),
        "full_price_value_yuan": round(full, 2),
        "plant_area": round(sum(row["area"] for row in rows), 2),
        "crop_count": len({row["crop_id"] for row in rows}),
        "audit": {key: value for key, value in audit(model).items() if not key.endswith("detail")},
    }
