...
赛题要求跨子图依赖图必须无环。把操作任意分到各核会构造出子图级的环，因此本模块
采用“依赖锥聚簇 + 无环约束局部搜索”两段式构造：

1. **依赖锥聚簇**：先按可达工作量找出每个操作的主前驱（承载最重上游分支的前驱），
   把全部源操作按工作量均衡地撒到各核，再让其余操作跟随主前驱所在核。这样同一
   依赖锥内的生产与消费天然落在同一核，跨核边只出现在多前驱汇合处。
2. **无环约束局部搜索**：以赛题 1.4 节口径的任务级估算完工时间为目标，把最重核上
   的节点迁往其他核；每次迁移后立即重建子图依赖图并检查无环性，只有既保持无环又
   降低目标函数的迁移才被接受。

估算模型：一个核的时长取该核矩阵流水线工作量、向量流水线工作量、入向与出向边界
搬运折算周期四者的最大值；跨核依赖再叠加固定等待。
"""
...
class GraphIndex:
    ...
    def __init__(self, graph):
        ...
        self.tensor_consumers = {
            tensor_id: sorted(c for c in consumers_of if c in self.eligible)
            for tensor_id, consumers_of in consumers.items()
        }
        self.cm = {}
        self.cv = {}
    ...
    def downstream_weight(self):
        """各操作可达工作量上界（反向拓扑动态规划，允许重复计数作为启发式权重）。"""
...
class PartitionState:
        ...
        for core in range(self.num_cores):
            result.append(
                max(
                    self.m_sum[core],
                    self.v_sum[core],
                    self.in_bytes[core] / self.in_bandwidth,
                    self.out_bytes[core] / BANDWIDTH,
                )
            )
        return result
    ...
    def subgraph_durations(self):
        """每个子图的估算时长：各流水线工作量、边界搬运与核内关键路径的最大值。

        关键路径在一次拓扑遍历内完成：只沿子图内部的依赖边累计，因此复杂度为
        $O(|V|+|E|)$，与子图个数无关。
        """
        index = self.index
        ...
        for subgraph in members:
            durations[subgraph] = max(
                m_sum[subgraph],
                v_sum[subgraph],
                in_bytes.get(subgraph, 0) / self.in_bandwidth,
                out_bytes.get(subgraph, 0) / BANDWIDTH,
                critical[subgraph],
            )
        return durations

    def _boundary_bytes(self):
        """按（子图，张量）对统计边界搬运：每个子图各自读入与写回，不跨子图去重。"""
    ...
    def task_objective(self, cross_wait, same_core_wait=100.0):
        ...
        取任务级流水估算与全局主存带宽下界（总搬运字节数除以主存带宽）的较大者：
        切分引入的边界搬运与原始搬运共享同一条主存带宽，当边界搬运量较大时带宽会
        成为真正的瓶颈，必须显式计入，否则会高估细切分的收益。
        """
...
def break_cycles(state: PartitionState, rounds=40):
    ...
    for _ in range(rounds):
        ...
        candidates = [
            node
            for node, core in state.core_of.items()
            if core == b and any(state.core_of.get(p) == a for p in index.cpreds[node])
        ]
        if not candidates:
            return state.acyclic()
...
def _block_state(index: GraphIndex, sequence, bounds, num_cores, cross_wait,
                 in_bandwidth: float = BANDWIDTH):
    """按给定切点构造连续段并做列表分核，返回分组状态与估算完工时间。"""
    stats = []
    block_of = {}
    ...
    while placed < num_cores:
        ...
        for core in range(num_cores):
            ...
            if best_finish is None or end < best_finish - 1e-9:
...
def refine(state: PartitionState, rounds=6, budget=6000):
    """以“最重核时长 + 跨核边惩罚”为目标削减跨核边，不要求保持无环。"""
    index = state.index
    evaluations = 0
    for _ in range(rounds):
        improved = False
        boundary = sorted(
            {
                node
                for (src, dst), count in state.cross.items()
                if count > 0
                for node in ()
            }
        )
        ...
        for node in boundary:
            ...
            for target in range(state.num_cores):
                ...
                state.move(node, target)
                evaluations += 1
                if _refine_score(state) < base - 1e-9:
                    improved = True
                    break
                state.move(node, origin)
        if not improved:
...
def level_assignment(index: GraphIndex, num_cores: int, band=1, balance_blocks=None,
                     in_bandwidth: float = BANDWIDTH):
    """分层切分：按最长路径分层，层内操作相互独立，层与层之间沿路径严格递增。

    把每个分层（或每 `band` 个连续分层组成的层带）内部按工作量切成若干块并均衡
    分配到各核。由于所有依赖边都从低层指向高层，子图编号按层递增即天然无环，
    核心分配不受无环性约束，因此可以在保持合法性的同时自由追求负载均衡。
    """
    level = {}
    for node in index.order:
        best = 0
...
def build_plan(graph, num_cores, cross_wait=1000.0, same_core_wait=100.0,
               in_bandwidth=BANDWIDTH, search_rounds=8, search_budget=3000,
               refine_rounds=3, refine_budget=2500, level_bands=(1, 2, 4),
               safety_margin=0.85):
    """主入口：多候选构造 + 估算择优 + 单核兜底。

    单核兜底保证算法在本质串行用例上不比单核基线更差：只有并行方案的估算完工
    时间比单核方案低出 `safety_margin` 以上时，才采用并行方案。取 0.85 是因为
    估算模型对边界搬运与缓存换出的刻画偏乐观，需要留出约百分之十五的余量。
    """
    ...
    core_schedules = [
        sorted(per_core.get(core, []), key=lambda s: first_pos[s]) for core in range(num_cores)
    ]
    return {
        "node_to_subgraph": {str(n): int(s) for n, s in sorted(node_to_subgraph.items())},
        "core_schedules": core_schedules,
    }


def _fallback_assignment(index: GraphIndex, num_cores: int, in_bandwidth: float = BANDWIDTH):
    """兜底构造：按“保持后代连续的拓扑序 + 连续段切分”保证商图无环。"""
    sequence = _descendant_order(index)
    total = sum(index.weight[n] for n in sequence)
    target = total / num_cores
    state = PartitionState(index, num_cores, in_bandwidth=in_bandwidth)
