...
所有实验都通过本模块进入官方评估链路，确保结果与赛题口径完全一致。
评估脚本本体位于赛题附件解压目录，本模块不修改其任何文件。解压目录采用惰性定位，
因此本模块可以在缺少赛题附件的环境中被安全导入（用于附录代码等价性回归）。
"""
...
def suite_dir() -> Path:
    ...
    if _SUITE is not None:
        return _SUITE
    candidates = [
        PROJECT_ROOT / "赛题" / "_附件解压",
        PROJECT_ROOT.parent / "_hw_suite",
    ]
...
def run_evaluator(problem, graph_path, plan_path, result_path=None, keep_trace=False):
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"evaluator failed (problem {problem}, {graph_path.name}): "
...
def run_singlecore(graph_path, result_path=None, keep_trace=False):
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"singlecore evaluator failed ({graph_path.name}): "
            f"{proc.stderr.strip() or proc.stdout.strip()}"
        )
    if not keep_trace:
        trace_path.unlink(missing_ok=True)
