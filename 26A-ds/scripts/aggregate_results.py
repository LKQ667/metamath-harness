"""把逐用例评估输出聚合为全项目唯一结果源 results/final_results.json。

所有论文、摘要、result.md 与 README 中的数值只能引用本文件。聚合过程不做任何
数值加工，只做求和、平均与比值，并保留逐用例明细以便复核。
"""
...
def result_for(case, problem, cores, suffix=""):
    if cores == 1:
        # 单核配置下三个场景退化为同一个任务，直接采用官方单核评估结果。
        candidates = [EVAL / f"{case}_singlecore_res.json"]
    else:
        candidates = [
            EVAL / f"{case}_p{problem}_n{cores}{suffix}_res.json",
            EVAL / f"{case}_p{problem}_n{cores}_res.json",
        ]
    ...
    for case, single in singles.items():
        ...
        for cores in CORES:
            ...
            item = {
                ...
                "speedup": 1.0 if cores == 1 else round(single / result["makespan"], 6),
            }
...
def estimator_scores(case, cores, problem):
    """从已保存方案复算估算完工时间，用于检查估算模型与实测的一致性。"""
...
def main():
    ...
    # 问题三：只读 L2 配置（L2 感知切图）与无 L2 基线（问题二结果）
    ...
    payload = {
        "meta": {
            "problem_title": "通用神经网络处理器下的多核调度问题",
            "competition": "华为杯（中国研究生数学建模竞赛）",
            "case_count": len(singles),
            "cores": CORES,
            "singlecore_source": "赛题官方 singlecore_evaluate.py",
            "evaluator_source": "赛题官方 multicore_cut_evaluate_problem_{1,2,3}.py",
            "config_source": "赛题官方 data/config.txt",
        },
        "ranges": {
            "speedup": {"min": 0.9, "max": 5.5},
            "cache_hit_rate": {"min": 0.0, "max": 1.0},
        },
        "results": {
            "key_result_q1_average_speedup_2core": q1["average_speedup"].get("2"),
            "key_result_q1_average_speedup_5core": q1["average_speedup"].get("5"),
            "key_result_q2_average_speedup_5core": q2["average_speedup"].get("5"),
            "key_result_q3_average_cache_gain_5core": gain.get("5"),
            "key_result_q3_cache_hit_percent_5core": round(
                float(hit_rate.get("5", 0.0)) * 100.0, 1
            ),
            "key_result_q3_cache_hit_rate_5core_fraction": hit_rate.get("5"),
        },
    ...
    print(
        json.dumps(
            {
                "cases": len(singles),
                "q1_speedup": q1["average_speedup"],
                "q2_speedup": q2["average_speedup"],
                "q3_gain": gain,
            },
            ensure_ascii=False,
        )
