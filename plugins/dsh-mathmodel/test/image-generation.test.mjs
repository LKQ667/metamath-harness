...
const secret = 'sk-image-secret-fx';
...
test('默认当前连接只调用一次并写入无秘密连接元数据', async () => {
  ...
  assert.match(metadata, /"connectionId": "img_ready_01"/); assert.match(metadata, /"protocol": "openai-images"/); assert.doesNotMatch(metadata, /绘制模型结构图|image-secret-fx|images\.example/);
});
