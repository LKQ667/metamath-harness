...
const IMAGE_TEMPLATE_META = Object.freeze([
  ...
  Object.freeze({ id: 'codex-subscription', name: 'ChatGPT 订阅', baseUrlEditable: false, defaultBaseUrl: '', defaultModel: 'gpt-image-2', adapters: ['codex-images'], defaultAdapter: 'codex-images', capability: 'pending', hint: '复用“设置 → 订阅”中的 Codex 登录，无需 API Key；走 ChatGPT 订阅 Codex 生图端点（gpt-image-2）。非官方支持端点，存在账号限制风险。' }),
  ...
  window.__ModuleLoader__.load({
    ...
    factory: (require) => {
      ...
      function hideMetaMathHeroFish() {
        ...
        if (hitbox && hitbox.dataset.dshMetamathHeroHidden !== 'true') {
          const officialFish = hitbox.querySelector('svg');
          if (officialFish) officialFish.style.display = 'none';
          hitbox.style.display = 'none';
          hitbox.dataset.dshMetamathHeroHidden = 'true';
        }
      }
      hideMetaMathHeroFish();
        ...
        const editorMarkup = editing ? (() => {
          ...
          return React.createElement('div', { className: 'dsh-mm-image-provider-editor', role: 'group', 'aria-label': '编辑生图连接' },
            ...
            React.createElement('div', { className: 'dsh-mm-image-provider-actions' },
              ...
              configured && editing.id && templateMeta(editing.draft.template)?.id !== 'codex-subscription' && templateMeta(editing.draft.template)?.id !== 'grok-subscription' ? React.createElement('button', { type: 'button', className: 'dsh-mm-image-provider-button', disabled: !editable, onClick: () => doClearKey(editing.id) }, '清除 Key') : null,
