...
/** 持久切换文件宠物显示状态；只写扫描到的配置文件，不接受调用方提供路径。 */
export async function setFilePetVisibility(
  paths: ConfigPaths,
  petId: string,
  visible: boolean,
  ...
  for (const file of scanPetFiles(paths.petDir)) {
    const parsed = readJsonc(file.path);
    const pets = Array.isArray(parsed?.pets) ? (parsed.pets as Record<string, unknown>[]) : [];
    ...
    if (visible) {
      const previous =
        typeof pet.dshPetDisplayBeforeClose === 'string' &&
        PET_DISPLAY_SET.has(pet.dshPetDisplayBeforeClose) &&
        pet.dshPetDisplayBeforeClose !== 'none'
          ? pet.dshPetDisplayBeforeClose
          : 'web';
