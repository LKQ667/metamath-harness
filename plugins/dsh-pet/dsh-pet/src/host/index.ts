...
function readBody(req: IncomingMessage, maxBytes = 260 * 1024 * 1024): Promise<string> {
...
export function apply(ctx: any): void {
  ...
  const handlePetRoute = async (rawUrl: string, method: string, body?: string): Promise<RouteResult> => {
    ...
    // 文件宠物导入：浏览器选择完整目录后，以相对路径 + base64 文件内容上传。
    if (rest === 'pet-pack/import') {
      if (method !== 'POST') return { kind: 'json', status: 405, obj: { error: 'method not allowed' } };
      try {
        const payload = JSON.parse(body ?? '') as PetPackUploadPayload;
        const existingPetIds = new Set(
          flattenPetList(readAllConfig(configPaths))
            .map((pet) => String(pet.id ?? ''))
            .filter(Boolean),
        );
        const result = await importPetPack(payload, { petDir: petConfigDir, existingPetIds });
        syncDesktop();
        return { kind: 'json', status: 200, obj: { ok: true, ...result } };
      } catch (error) {
        if (error instanceof PetPackImportError) {
          return { kind: 'json', status: error.status, obj: { error: error.message } };
        }
        return {
          kind: 'json',
          status: 500,
    ...
    if (rest === 'pet/visibility') {
      if (method !== 'POST') return { kind: 'json', status: 405, obj: { error: 'method not allowed' } };
      try {
        const payload = JSON.parse(body ?? '') as { petId?: unknown; visible?: unknown };
        if (typeof payload.petId !== 'string' || typeof payload.visible !== 'boolean') {
          return { kind: 'json', status: 400, obj: { error: 'invalid pet visibility payload' } };
        }
        const result = await setFilePetVisibility(configPaths, payload.petId, payload.visible);
        if (!result) return { kind: 'json', status: 404, obj: { error: 'file pet not found' } };
        syncDesktop();
        return { kind: 'json', status: 200, obj: { ok: true, ...result } };
      } catch (error) {
        return {
