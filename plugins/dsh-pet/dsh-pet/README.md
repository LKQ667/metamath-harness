...
也可以在 DSH「设置 → 桌宠配置」点击**导入宠物包**，一次选择包含下列两个条目的完整文件夹。导入会先检查目录结构、配置引用、宠物 id、文件类型和大小；同名前缀默认拒绝覆盖。`pet-2`、`pet-3` 等按钮只是主宠物实例切换，不是文件选择按钮。

导入后的文件宠物会以“文件”标记出现在宠物列表；点击后可用“关闭宠物 / 显示宠物”持久切换显示状态，其余字段仍由对应配置文件维护。
