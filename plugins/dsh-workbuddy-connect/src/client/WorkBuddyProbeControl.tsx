...
export function WorkBuddyProbeControl({ directory, t }: WorkBuddyProbeControlProps) {
  ...
  // DSH 0.1.5 起目录快照以 `null` 表示“尚无选中模型”（0.1.2 为 `undefined`）；
  // 归一为 undefined，后续判空逻辑保持不变。
  const selection = useSyncExternalStore(subscribe, snapshot, snapshot).current ?? undefined
