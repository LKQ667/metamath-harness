...
const DESKTOP_AUTH_DIR = ['CodeBuddyExtension', 'Data', 'Public', 'auth'] as const
// Local derivation (metamath): current WorkBuddy desktop builds write the auth
// file with an `-ai` suffix. Probe both names so a machine-specific absolute
// `authFile` override is never required.
const DESKTOP_AUTH_FILENAMES = ['workbuddy-desktop.info', 'workbuddy-desktop-ai.info'] as const

const DESKTOP_AUTH_RELATIVE_PATHS = DESKTOP_AUTH_FILENAMES.map((filename) => [...DESKTOP_AUTH_DIR, filename])
...
export function defaultDesktopAuthCandidates(): string[] {
  ...
  if (process.platform === 'darwin') {
    return DESKTOP_AUTH_RELATIVE_PATHS.map((relativePath) => join(home, 'Library', 'Application Support', ...relativePath))
  }
  if (process.platform === 'win32') {
    return [join(home, 'AppData', 'Local'), join(home, 'AppData', 'Roaming')].flatMap((authRoot) =>
      DESKTOP_AUTH_FILENAMES.map((filename) => join(authRoot, ...DESKTOP_AUTH_DIR, filename)),
    )
  }
