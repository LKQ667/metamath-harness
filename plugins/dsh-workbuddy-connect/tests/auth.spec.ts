...
describe('Windows default desktop path probing', () => {
  ...
  async function fakeWindowsHome(): Promise<{ home: string, local: string, localAi: string, roaming: string, roamingAi: string }> {
    const home = await mkdtemp(join(tmpdir(), 'wb-win-'))
    CLEANUP.push(() => rm(home, { recursive: true, force: true }))
    const authDir = join('CodeBuddyExtension', 'Data', 'Public', 'auth')
    const local = join(home, 'AppData', 'Local', authDir, 'workbuddy-desktop.info')
    const localAi = join(home, 'AppData', 'Local', authDir, 'workbuddy-desktop-ai.info')
    const roaming = join(home, 'AppData', 'Roaming', authDir, 'workbuddy-desktop.info')
    const roamingAi = join(home, 'AppData', 'Roaming', authDir, 'workbuddy-desktop-ai.info')
    return { home, local, localAi, roaming, roamingAi }
  }
  ...
  it('lists Local before Roaming on win32', async () => {
    const { home, local, localAi, roaming, roamingAi } = await fakeWindowsHome()
    await asWindows(home, async () => {
      const candidates = defaultDesktopAuthCandidates()
      expect(candidates).toEqual([local, localAi, roaming, roamingAi])
    })
  })
